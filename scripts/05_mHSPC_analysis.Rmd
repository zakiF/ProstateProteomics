...
```{r}
# Wide matrix: Assay x sample
heatmap_data <- df_plot %>%
  select(Assay, final_sample_id, NPX) %>%
  pivot_wider(names_from = final_sample_id, values_from = NPX) %>%
  column_to_rownames("Assay") %>%
  as.matrix()

annotation_col <- df_plot %>%
  select(final_sample_id, txt_stat) %>%
  distinct() %>%
  column_to_rownames("final_sample_id") %>%
  rename(Group = txt_stat)

annotation_col <- annotation_col[colnames(heatmap_data), , drop = FALSE]
...
col_dist <- dist(t(heatmap_data), method = "euclidean")
col_hclust <- hclust(col_dist, method = "complete")

col_dend <- as.dendrogram(col_hclust)

desired_order <- rownames(annotation_col)[
  order(annotation_col$Group)
]

col_dend <- rotate(col_dend, desired_order)


# Desired order: Pre first, then Post
desired_order <- rownames(annotation_col)[
  order(annotation_col$Group)
]
...
col_hclust_rot <- as.hclust(col_dend)
...
pheatmap(
  ...
  annotation_col = annotation_col,
  annotation_colors = annotation_colors,
  cluster_cols = col_hclust_rot,   # <-- key line
  show_rownames = TRUE,
  show_colnames = FALSE,
  clustering_distance_rows = "euclidean",
  clustering_distance_cols = "euclidean",
  ...
  main = "Pre vs Post Treatment",
  fontsize_row = 8,
  fontsize_col = 8
)

pdf(file.path(wd$outCurr, "Pre_post_heatmap.pdf"), height = 4, width = 9)
pheatmap(
  heatmap_data,
  annotation_col = annotation_col,
  annotation_colors = annotation_colors,
      ...
      if(exists("df_plot_mcrpc")) {
        ...
        for(protein_id in mets_protein_ids) {
          ...
          if(length(enza_post) >= 3 & length(all_mcrpc_post) >= 3) {
            ...
            cat(protein_name, " treatment effect: Enzalutamide Δ =", round(enza_change, 3),
                " vs All mCRPC Δ =", round(all_change, 3), "\n")
          }
        }
      }
