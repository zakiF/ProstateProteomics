...
```{r}
un_1 <- df_meta_f4 %>% filter(sample_id_psom %in% dat1_final$SampleID) %>% filter(Psomagen == "Y") %>% pull(HCI_cID)
un_2 <- df_meta_f4 %>% filter(sample_id_Q13356 %in% dat2_final$SampleID) %>% filter(Q_13356 == "Y") %>% pull(HCI_cID)
un_3 <- df_meta_f4 %>% filter(sample_id_Q15806 %in% dat3_final$SampleID) %>% filter(Q_15806 == "Y") %>% pull(HCI_cID)
...
overlap <- calculate.overlap(
x <- list("Psomagen"=un_1, 
          "Q_13356"=un_2,
          "Q_15806"=un_3))

pdf(file=file.path(wd$outCurr, "Samples_venn_overlap_filtered.pdf"), width = 5, height = 5)
ggvenn(
  x, 
  fill_color = pal.study,
  show_percentage = FALSE,
...
un_1 <- df_meta_f4 %>% filter(sample_id_psom %in% dat1_final$SampleID) %>% filter(Psomagen == "Y") %>% pull(mrn)
un_2 <- df_meta_f4 %>% filter(sample_id_Q13356 %in% dat2_final$SampleID) %>% filter(Q_13356 == "Y") %>% pull(mrn)
un_3 <- df_meta_f4 %>% filter(sample_id_Q15806 %in% dat3_final$SampleID) %>% filter(Q_15806 == "Y") %>% pull(mrn)
...
pdf(file=file.path(wd$outCurr, "Patient_venn_overlap_filtered.pdf"), width = 5, height = 5)
ggvenn(
  x, 
  fill_color = pal.study,
  show_percentage = FALSE,
...
vars <- apply(m_all, 1, var, na.rm = TRUE)
top_features <- names(sort(vars, decreasing = TRUE))[1:1000]
m_top <- m_all[top_features, ]
pca_before <- prcomp(t(m_top), scale. = TRUE)
...
vars <- apply(mat_in, 1, var, na.rm = TRUE)
top_features <- names(sort(vars, decreasing = TRUE))[1:1000]
m_top <- mat_in[top_features, ]
pca_after <- prcomp(t(m_top), scale. = TRUE)
df_pca2 <- pca_after$x %>% as.data.frame() %>% 
...
# Create labels for plotting
pc1_labe_a2 <- paste0("PC1 (", round(pc1_variance, 2), "%)")
pc2_label_a2 <- paste0("PC2 (", round(pc2_variance, 2), "%)")


p2.1 <- 
ggplot(df_pca2, aes(x = PC1_after, y = PC2_after, color = Batch)) +
  geom_point(size = pt_sz) +
  labs(
...
p2.2 <- 
ggplot(df_pca2, aes(x = PC1_after, y = PC2_after, color = cohort)) +
  geom_point(size = pt_sz) +
  labs(
    title = "PCA After Limma Batch Correction",
    x = pc1_labe_a2,
...
n_total_samples <- df_meta_in %>% pull(HCI_cID) %>% unique() %>% length()
n_total_patients <- df_meta_in %>% pull(mrn) %>% unique() %>% length()


# Calculate total number of samples and patients for Cohorts A, B, and C (regardless of treatment)
n_a_samples <- df_meta_in %>% filter(cohort == "A") %>% pull(HCI_cID) %>% unique() %>% length()
n_a_patients <- df_meta_in %>% filter(cohort == "A") %>% pull(mrn) %>% unique() %>% length()

n_b_samples <- df_meta_in %>% filter(cohort == "B") %>% pull(HCI_cID) %>% unique() %>% length()
n_b_patients <- df_meta_in %>% filter(cohort == "B") %>% pull(mrn) %>% unique() %>% length()

n_c_samples <- df_meta_in %>% filter(cohort == "C") %>% pull(HCI_cID) %>% unique() %>% length()
n_c_patients <- df_meta_in %>% filter(cohort == "C") %>% pull(mrn) %>% unique() %>% length()
...
Save a corrected matrix, renamed according to the HCI_cID

```{r}
mat_exp <- exp_mat_sub
# Add letter to the sample name so R does not have potential errors
meta_olink <- df_final_ids %>%
  mutate(
    HCI_cID = paste0("c", gsub("-", "_", HCI_cID))
  )
...
# colnames(mat_exp) <- lookup[colnames(mat_exp)]

# Create reverse mapping: Disc_29 -> c2021_3867
rev_lookup <- setNames(names(lookup), lookup)

# Rename columns
colnames(mat_exp) <- rev_lookup[colnames(mat_exp)]

# Optional: check result
head(colnames(mat_exp))

write.csv(mat_exp, file.path(wd$outData, "ExpMat_integrated.csv"))

saveRDS(mat_exp, file.path(wd$outData, "Olink_expMat.RDS"))
saveRDS(meta_olink, file.path(wd$outData, "Olink_Meta.RDS"))
```
