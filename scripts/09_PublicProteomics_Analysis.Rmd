...
if (nrow(matched_proteins) > 0) {
  ...
  olink_long <- olink_expr %>%
    ...
    left_join(olink_meta %>% select(HCI_cID, cohort, txt_stat_ordered), by = "HCI_cID") %>%
...
This might contain duplicates. Do a simple venn

```{r}
un_a <- unique(olink_filtered$Assay)
un_b <- unique(prot_data_with_gene$Gene_Name)
...
> Ticky part - some prteins matched to mulitple fold change
...
log2FC heatmap showing Olink (plasma) and public SWATH-MS (tissue) fold changes for all matched proteins, arranged by combined log2FC.

```{r log2fc_heatmap, fig.width=10, fig.height=14, warning=FALSE, message=FALSE}
prog_pro <- res_all2 %>% dplyr::filter(Multi_Sig == "Yes") %>% pull(OlinkID)

if (exists("concordance_df") && nrow(concordance_df) > 0) {

  # Prepare heatmap data
  heatmap_pre <- concordance_df %>%
    filter(!is.na(Olink_log2FC), !is.na(Public_log2FC)) %>%
    mutate(Combined_log2FC = Olink_log2FC + Public_log2FC) %>%
    dplyr::filter(OlinkID %in% prog_pro) # <---------------- TO CHANGE IF WE WANT TO SHOW ALL HEATMAP

  # Identify duplicated Assay names
  duplicated_assays <- heatmap_pre %>%
    group_by(Assay) %>%
    filter(n() > 1) %>%
    arrange(Assay) %>%
    select(Assay, OlinkID, Olink_log2FC, Public_log2FC, Combined_log2FC, Concordant)

  cat("\n=== Duplicated Assay Names ===\n")
  cat("Total rows before dedup:", nrow(heatmap_pre), "\n")
  cat("Duplicated genes:", n_distinct(duplicated_assays$Assay), "\n\n")
  print(duplicated_assays)

  # Deduplicate by Assay — keep row with highest Combined_log2FC per gene
  heatmap_dedup <- heatmap_pre %>%
    group_by(Assay) %>%
    slice_max(Combined_log2FC, n = 1, with_ties = FALSE) %>%
    ungroup()

  # Order by tissue log2FC (Public_log2FC):
  #   Concordant (positive Public_log2FC): descending — highest tissue FC at top
  #   Discordant (negative Public_log2FC): ascending  — most negative at bottom
  #   → creates a smooth gradient from high positive (top) to most negative (bottom)
  concordant_part <- heatmap_dedup %>%
    filter(Concordant == TRUE) %>%
    arrange(desc(Public_log2FC))

  discordant_part <- heatmap_dedup %>%
    filter(Concordant == FALSE) %>%
    arrange(desc(Public_log2FC))

  heatmap_df <- bind_rows(concordant_part, discordant_part) %>%
    mutate(Assay = factor(Assay, levels = rev(Assay)))  # rev() puts first row at top

  cat("\nProteins after deduplication:", nrow(heatmap_df), "\n")

  cat("Proteins in heatmap:", nrow(heatmap_df), "\n")
  cat("  Concordant:", sum(heatmap_df$Concordant), "\n")
  cat("  Discordant:", sum(!heatmap_df$Concordant), "\n")

  # Long format for heatmap (2 columns)
  heatmap_long <- heatmap_df %>%
    select(Assay, Concordant,
           `Olink (Plasma)` = Olink_log2FC,
           `SWATH-MS (Tissue)` = Public_log2FC) %>%
    pivot_longer(cols = c(`Olink (Plasma)`, `SWATH-MS (Tissue)`),
                 names_to = "Dataset",
                 values_to = "log2FC") %>%
    mutate(Dataset = factor(Dataset, levels = c("Olink (Plasma)", "SWATH-MS (Tissue)")))

  # --- Plot 1: Concordance annotation bar (left) ---
  p_annot <- ggplot(heatmap_df, aes(x = "Direction", y = Assay, fill = Concordant)) +
  ...
  # --- Plot 2: Two-column log2FC heatmap (right) ---
  p_heatmap <- ggplot(heatmap_long, aes(x = Dataset, y = Assay, fill = log2FC)) +
    geom_tile(color = "white", linewidth = 0.3) +
  ...
  p_heatmap_combined <- p_annot + p_heatmap +
    plot_layout(widths = c(0.15, 1)) +
    plot_annotation(
      title = paste0("log2FC: Olink vs SWATH-MS (", nrow(heatmap_df), " matched proteins)"),
      subtitle = "Arranged by combined log2FC (highest to lowest). Left bar: green = concordant, red = discordant.",
  ...
  # Save
  ggsave(file.path(wd$outCurr, "Heatmap_log2FC_Concordance_Olink_vs_Public.pdf"),
         plot = p_heatmap_combined, width = 10, height = 14)

  cat("\nHeatmap saved to: Heatmap_log2FC_Concordance_Olink_vs_Public.pdf\n")

} else {
  cat("concordance_df not available\n")
}
```

Flipped version

```{r log2fc_heatmap_flipped, fig.width=20, fig.height=6, warning=FALSE, message=FALSE}

if (exists("concordance_df") && nrow(concordance_df) > 0) {

  # Reuse heatmap_df / heatmap_long built in the chunk above
  # (prog_pro, heatmap_dedup, concordant_part, discordant_part, heatmap_df, heatmap_long
  #  are all still in the environment from the previous chunk)

  # --- Plot 1: Two-row log2FC heatmap (top) ---
  # Proteins on x-axis, datasets on y-axis.
  # scale_x_discrete(limits = rev) reverses the x-axis so the highest-FC
  # protein (top of the original plot) ends up on the LEFT.
  p_heatmap_h <- ggplot(heatmap_long, 
                        aes(x = Assay, y = Dataset, fill = log2FC)) +
    geom_tile(color = "white", linewidth = 0.3) +
  ...
  # --- Plot 2: Concordance annotation bar (bottom) ---
  # Must use the same scale_x_discrete(limits = rev) so columns align.
  p_annot_h <- ggplot(heatmap_df, aes(x = Assay, y = "Direction", fill = Concordant)) +
    geom_tile(color = "white", linewidth = 0.3) +
    scale_x_discrete(limits = rev) +
    scale_fill_manual(values = c("TRUE" = "#2ca02c", "FALSE" = "#d62728"),
                      labels = c("TRUE" = "Concordant", "FALSE" = "Discordant"),
  ...
  # --- Combine: heatmap on top, annotation strip on bottom ---
  p_heatmap_combined_h <- p_heatmap_h / p_annot_h +
    plot_layout(heights = c(1, 0.15)) +
    plot_annotation(
      title    = paste0("log2FC: Olink vs SWATH-MS (", nrow(heatmap_df), " matched proteins)"),
      subtitle = "Proteins on x-axis, ordered highest→lowest FC (left→right). Bottom bar: green = concordant, red = discordant.",
      theme    = theme(plot.title    = element_text(size = 14, face = "bold"),
                       plot.subtitle = element_text(size = 10))
    )

  print(p_heatmap_combined_h)

  # Save
  ggsave(file.path(wd$outCurr, "Heatmap_log2FC_Concordance_Olink_vs_Public_horizontal.pdf"),
         plot = p_heatmap_combined_h, width = 20, height = 6)

  cat("\nHeatmap saved to: Heatmap_log2FC_Concordance_Olink_vs_Public_horizontal.pdf\n")
...
### Platform Dominance: Diverging Bar Chart

For each protein, the bar direction indicates which platform shows relatively greater upregulation in mCRPC. The metric is the **z-score normalised difference** (`z(Public_log2FC) - z(Olink_log2FC)`), which places both platforms on a common scale before comparing:

- **Left (negative)** → tissue (SWATH-MS) fold change is relatively higher
- **Right (positive)** → plasma (Olink) fold change is relatively higher
- **Bar colour** → Combined log2FC (overall signal strength across both platforms)

```{r diverging_bar, fig.width=7, fig.height=14, warning=FALSE, message=FALSE}
if (exists("concordance_df") && nrow(concordance_df) > 0) {

  # Build data — deduplicate and compute metrics
  divbar_df <- concordance_df %>%
    filter(!is.na(Olink_log2FC), !is.na(Public_log2FC)) %>%
    mutate(Combined_log2FC = Olink_log2FC + Public_log2FC) %>%
    group_by(Assay) %>%
    slice_max(Combined_log2FC, n = 1, with_ties = FALSE) %>%
    ungroup() %>%
    mutate(
      # Z-score each platform's log2FC independently (removes scale difference)
      z_Olink  = as.numeric(scale(Olink_log2FC)),
      z_Public = as.numeric(scale(Public_log2FC)),
      # Positive = tissue dominates, Negative = plasma dominates
      Platform_Diff = z_Public - z_Olink
    ) %>%
    arrange(Platform_Diff) %>%                         # plasma → left, tissue → right
    mutate(Assay = factor(Assay, levels = Assay))      # lock order

  # Bar chart
  p_divbar <- ggplot(divbar_df,
                     aes(x = Platform_Diff, y = Assay, fill = Combined_log2FC)) +
    geom_bar(stat = "identity", width = 0.8) +
    geom_vline(xintercept = 0, color = "black", linewidth = 0.6) +
    scale_fill_gradient(low = "#fee8c8", high = "#b30000",
                        name = "Combined\nlog2FC") +
    scale_x_continuous(
      name = "← Higher in Tissue (SWATH-MS)          Higher in Plasma (Olink) →"
    ) +
    labs(title = "Platform Dominance per Protein",
         subtitle = "z-normalised difference: z(Public log2FC) − z(Olink log2FC)",
         y = NULL) +
    theme_bw() +
  ...
  # Save
  ggsave(file.path(wd$outCurr, "DivergingBar_PlatformDominance.pdf"),
         plot = p_divbar, width = 7, height = 14)

  cat("\nDiverging bar chart saved to: DivergingBar_PlatformDominance.pdf\n")

  # Summary
  n_tissue  <- sum(divbar_df$Platform_Diff > 0)
  n_plasma  <- sum(divbar_df$Platform_Diff < 0)
  cat(sprintf("Proteins with relatively higher tissue FC:  %d\n", n_tissue))
  cat(sprintf("Proteins with relatively higher plasma FC:  %d\n", n_plasma))
...
if (nrow(matched_proteins) > 0 && requireNamespace("ggbeeswarm", quietly = TRUE)) {
  ...
  # Deduplicate: keep one unique SWATH-MS Protein ID per gene (Assay)
  matched_proteins_dedup <- matched_proteins %>%
    distinct(Assay, .keep_all = TRUE)

  cat("matched_proteins after dedup:", nrow(matched_proteins_dedup),
      "(from", nrow(matched_proteins), "before)\n")

  # Get matched protein data from normalized matrix
  matched_protein_ids <- matched_proteins_dedup$Protein
  ...
  cat("Scaled data prepared:", nrow(matched_proteins_dedup), "proteins x",
...
if (exists("concordance_df") && nrow(concordance_df) > 0 &&
  ...
  if (nrow(top2_concordant) >= 2) {
    ...
    p1_top2 <- ggplot(olink_top2, aes(x = Group, y = Expression, fill = Group)) +
      geom_quasirandom(dodge.width = 0.9, colour = "grey30", alpha = 0.5, size = 2.5) +
      geom_violin(alpha = 0.5, scale = "width") +
      geom_boxplot(outlier.alpha = 0, width = 0.2, coef = 0) +
