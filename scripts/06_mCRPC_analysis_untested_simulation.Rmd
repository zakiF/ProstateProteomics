...
```{r}
df_plot <- res_all2 %>%
  select(Assay, OlinkID, Multi_HR, Multi_FDR_num, DrugBank.ID, is_CS) %>% 
...
available_proteins <- res_all %>% 
  ...
  filter(Assay %in% custom_proteins_of_interest$Assay)
...
cat("=== Top 5 Proteins for CPPS Construction ===\n")
...
## Function: Generate CPPS KM Plot from Custom Protein Set

```{r}
#' Generate CPPS Score and KM Plot for Custom Protein Set
#'
#' @param proteins Character vector of protein identifiers (either Assay names or OlinkIDs starting with OID)
#' @param protein_data Data frame containing protein information with Multi_Coefficient column (default: res_all2)
#' @param expression_data Long-format expression data (default: df_npx_long)
#' @param metadata Sample metadata with survival information (default: meta_in)
#' @param plot_title Optional custom plot title
#' @param save_plot Logical, whether to save plot as PDF (default: FALSE)
#' @param output_dir Directory to save plot if save_plot=TRUE (default: wd$outCurr)
#' @param verbose Logical, whether to print detailed output (default: TRUE)
#'
#' @return A list containing:
#'   - km_plot: ggsurvplot object
#'   - cpps_data: data frame with CPPS scores and survival data
#'   - statistics: list with HR, p-values, and other stats
#'   - protein_info: data frame with protein details and coefficients
#'
#' @details Uses multivariable Cox coefficients (Multi_Coefficient) from protein_data
#'   to calculate risk scores, matching the CPPS calculation approach.
#'
generate_cpps_km_plot <- function(proteins,
                                   protein_data = res_all2,
                                   expression_data = df_npx_long,
                                   metadata = meta_in,
                                   plot_title = NULL,
                                   save_plot = FALSE,
                                   output_dir = wd$outCurr,
                                   verbose = TRUE) {

  # Determine if input is Assay names or OlinkIDs
  is_olinkid <- all(grepl("^OID", proteins))

  if(verbose) {
    cat("\n===========================================\n")
    cat("Generating CPPS KM Plot\n")
    cat("===========================================\n")
    cat("Number of proteins:", length(proteins), "\n")
    cat("Input type:", ifelse(is_olinkid, "OlinkID", "Assay names"), "\n")
  }

  # Get protein information
  if(is_olinkid) {
    protein_info <- protein_data %>%
      filter(OlinkID %in% proteins)
  } else {
    protein_info <- protein_data %>%
  ...
  if(nrow(protein_info) == 0) {
    stop("No matching proteins found in protein_data!")
  }

  if(nrow(protein_info) < length(proteins)) {
    missing_count <- length(proteins) - nrow(protein_info)
    warning(paste(missing_count, "proteins not found in protein_data"))
  }

  if(verbose) {
    cat("\nMatched proteins:\n")
    print(protein_info %>% select(Assay, OlinkID, Multi_Coefficient, Multi_HR))
  }

  # Calculate CPPS score using multivariable coefficients
  cpps_scores <- expression_data %>%
    filter(OlinkID %in% protein_info$OlinkID) %>%
    select(final_sample_id, OlinkID, NPX) %>%
    left_join(protein_info %>% select(OlinkID, Multi_Coefficient), by = "OlinkID") %>%
    group_by(final_sample_id) %>%
    mutate(RS = Multi_Coefficient * NPX,
           Sum_rs = sum(RS, na.rm = TRUE)) %>%
    select(final_sample_id, Sum_rs) %>%
    distinct() %>%
    rename(sample_id = final_sample_id,
           CPPS = Sum_rs)
  ...
  # Merge with survival data
  cpps_data <- cpps_scores %>%
    left_join(metadata %>% select(sample_id, death, OS), by = "sample_id") %>%
    rename(SampleID = sample_id,
           time_to_death = OS) %>%
    na.omit()

  if(nrow(cpps_data) == 0) {
    stop("No samples with complete CPPS and survival data!")
  }

  # Create risk groups using median split
  cpps_median <- median(cpps_data$CPPS, na.rm = TRUE)
  cpps_data <- cpps_data %>%
    mutate(
      CPPS_risk = ifelse(CPPS >= cpps_median, "High", "Low"),
      CPPS_risk = factor(CPPS_risk, levels = c("Low", "High"))
  ...
  if(verbose) {
    ...
    print(table(cpps_data$CPPS_risk))
    cat("\nCPPS Statistics:\n")
    cat("  Median CPPS:", round(cpps_median, 3), "\n")
    cat("  Range:", round(min(cpps_data$CPPS), 3), "to", round(max(cpps_data$CPPS), 3), "\n")
  }

  # Log-rank test
  survdiff_result <- survdiff(Surv(time_to_death, death) ~ CPPS_risk, data = cpps_data)
  logrank_p <- 1 - pchisq(survdiff_result$chisq, length(survdiff_result$n) - 1)

  if(verbose) {
    cat("\nLog-rank Test:\n")
    print(survdiff_result)
    cat("Log-rank p-value:", formatC(logrank_p, format = "e", digits = 3), "\n")
  }

  # Fit Cox proportional hazards model
  cox_model <- coxph(Surv(time_to_death, death) ~ CPPS_risk, data = cpps_data)
  cox_summary <- summary(cox_model)

  # Extract HR and CI
  hr_value <- cox_summary$conf.int[1, "exp(coef)"]
  ...
  survfit_result <- survfit(Surv(time_to_death, death) ~ CPPS_risk, data = cpps_data)

  if(verbose) {
    cat("\nMedian Survival Times:\n")
    print(summary(survfit_result)$table)
  }

  # Create plot title
  if(is.null(plot_title)) {
    plot_title <- paste0("CPPS from ", length(proteins), " Proteins\n",
                        "HR = ", round(hr_value, 2),
  ...
  if(save_plot) {
    protein_string <- paste(proteins, collapse = "_")
    if(nchar(protein_string) > 50) {
      protein_string <- paste0(length(proteins), "proteins")
    }
    filename <- file.path(output_dir, paste0("CPPS_KM_custom_", protein_string, ".pdf"))
  ...
  results <- list(
    ...
    statistics = list(
      ...
      hr_p = hr_p,
      logrank_p = logrank_p,
      n_samples = nrow(cpps_data),
      n_events = sum(cpps_data$death),
      cpps_median = cpps_median
    ),
...
```{r}
# Example 1: Using Assay names
# example_proteins_assay <- c("KLK3", "FOLR1", "CRISP3", "MSMB", "WFDC2")
# result1 <- generate_cpps_km_plot(example_proteins_assay)
# print(result1$km_plot)

# Example 2: Using OlinkIDs
example_proteins_oid <- c("OID21319"	, "OID31356", "OID21144",	 "OID20272"	 ,"OID31062")
result2 <- generate_cpps_km_plot(example_proteins_oid, save_plot = FALSE)

# Example 3: Custom plot title and less verbose
# result3 <- generate_cpps_km_plot(
#   proteins = c("KLK3", "FOLR1", "CRISP3"),
#   plot_title = "Custom Protein Signature\nHR = X.XX [X.XX-X.XX]",
#   verbose = FALSE
# )
```
...
for(n_proteins in protein_counts) {
  ...
  # Calculate C-index (concordance index) for discrimination
  cindex_obj <- concordance(results$cox_model)
...
for(n_proteins in protein_counts) {
  ...
  sim_proteins <- vector("list", n_simulations)  # Store selected proteins
  ...
  for(sim in 1:n_simulations) {
    ...
    sim_proteins[[sim]] <- selected_proteins  # Save the selected proteins

    # Get protein info with multivariable coefficients from res_all2
    selected_protein_info <- res_all2 %>%
      filter(OlinkID %in% selected_proteins)

    # Calculate CPPS-style risk score using multivariable coefficients
    # This matches the CPPS calculation approach
    risk_score_df <- df_npx_long %>%
      filter(OlinkID %in% selected_proteins) %>%
      select(final_sample_id, OlinkID, NPX) %>%
      left_join(selected_protein_info %>% select(OlinkID, Multi_Coefficient), by = "OlinkID") %>%
      group_by(final_sample_id) %>%
      mutate(RS = Multi_Coefficient * NPX,
             Sum_rs = sum(RS, na.rm = TRUE)) %>%
      select(final_sample_id, Sum_rs) %>%
    ...
    # Merge with survival data
    sim_df_temp <- risk_score_df %>%
      left_join(meta_in %>% select(sample_id, OS, death), by = "sample_id") %>%
      filter(!is.na(OS) & !is.na(death))

    # Extract risk scores for binarization
    risk_scores <- sim_df_temp$risk_score
    names(risk_scores) <- sim_df_temp$sample_id
    ...
    if(nrow(sim_df) > 10 && sum(sim_df$death) > 2) {
      ...
      sim_cindex[sim] <- cindex_result[1]

      # Log-rank test using survminer's approach (same as ggsurvplot)
      # Create survival fit and use survminer's .get_pvalue function
      temp_survfit <- survfit(Surv(time_to_death, death) ~ risk_group, data = sim_df)
  ...
  simulation_results[[as.character(n_proteins)]] <- data.frame(
    ...
    proteins = I(sim_proteins),  # Store as list column
    stringsAsFactors = FALSE
  )
...
# Identify representative simulations (closest to mean_logrank_p) and show their proteins
cat("\n=== Representative Simulations (Closest to Mean Log-rank P-value) ===\n")

representative_sims <- data.frame()

for(n_prot in protein_counts) {
  # Get mean logrank_p for this protein count
  mean_logrank_p_val <- sim_summary %>%
    filter(n_proteins == n_prot) %>%
    pull(mean_logrank_p)

  # Find simulation closest to mean logrank_p
  sim_subset <- all_sim_results %>%
    filter(n_proteins == n_prot & !is.na(logrank_p))

  if(nrow(sim_subset) > 0) {
    closest_idx <- which.min(abs(sim_subset$logrank_p - mean_logrank_p_val))
    closest_sim <- sim_subset[closest_idx, ]

    # Get the proteins used in this simulation
    protein_list <- closest_sim$proteins[[1]]

    # Get protein names if available
    if(exists("df_id_pro")) {
      protein_names <- df_id_pro %>%
        filter(OlinkID %in% protein_list) %>%
        pull(Assay)
      protein_display <- paste(protein_names, collapse = ", ")
    } else {
      protein_display <- paste(protein_list, collapse = ", ")
    }

    cat("\n", n_prot, "proteins:\n", sep = "")
    cat("  Simulation ID:", closest_sim$simulation, "\n")
    cat("  Log-rank p-value:", round(closest_sim$logrank_p, 4),
        "(Mean:", round(mean_logrank_p_val, 4), ")\n")
    cat("  HR:", round(closest_sim$hr, 3),
        "[", round(closest_sim$hr_lower, 3), "-", round(closest_sim$hr_upper, 3), "]\n")
    cat("  C-index:", round(closest_sim$c_index, 3), "\n")
    cat("  Proteins:", protein_display, "\n")

    # Store for potential export
    representative_sims <- rbind(representative_sims, data.frame(
      n_proteins = n_prot,
      simulation = closest_sim$simulation,
      logrank_p = closest_sim$logrank_p,
      mean_logrank_p = mean_logrank_p_val,
      hr = closest_sim$hr,
      hr_lower = closest_sim$hr_lower,
      hr_upper = closest_sim$hr_upper,
      c_index = closest_sim$c_index,
      proteins_list = I(list(protein_list))
    ))
  }
}

# Save representative simulations info
write.csv(representative_sims %>% select(-proteins_list),
          file.path(wd$outCurr, "representative_simulations_summary.csv"),
          row.names = FALSE)

cat("\n=== Representative simulations summary saved ===\n")
...
### Simulation with Truly Null Proteins (Weak Coefficients)

To provide a more stringent negative control, we test proteins that are both non-prognostic (Multi_FDR > 0.2) AND have weak effect sizes (|coefficient| < 0.15, below median for non-prognostic proteins). These represent proteins with minimal association to mCRPC survival.

```{r}
set.seed(456)  # Different seed from main simulation

# Get list of truly null proteins (non-prognostic AND weak coefficients)
truly_null_proteins <- res_all2 %>%
  filter(Multi_FDR_num > 0.2 & abs(Multi_Coefficient) < 0.15) %>%
  pull(OlinkID)

cat("=== Truly Null Proteins Available ===\n")
cat("Total truly null proteins (Multi_FDR > 0.2 & |coef| < 0.15):", length(truly_null_proteins), "\n")
cat("\nCoefficient comparison:\n")
cat("  Prognostic mean |coef|:",
    round(mean(abs(res_all2$Multi_Coefficient[res_all2$Multi_FDR_num <= 0.2])), 3), "\n")
cat("  Non-prognostic mean |coef|:",
    round(mean(abs(res_all2$Multi_Coefficient[res_all2$Multi_FDR_num > 0.2])), 3), "\n")
cat("  Truly null mean |coef|:",
    round(mean(abs(res_all2$Multi_Coefficient[res_all2$OlinkID %in% truly_null_proteins])), 3), "\n")

# Run simulations with truly null proteins
n_simulations_null <- 100
null_simulation_results <- list()

for(n_proteins in protein_counts) {
  cat("\n=== Simulating with", n_proteins, "truly null proteins ===\n")

  sim_cindex <- numeric(n_simulations_null)
  sim_logrank_p <- numeric(n_simulations_null)
  sim_hr <- numeric(n_simulations_null)
  sim_hr_lower <- numeric(n_simulations_null)
  sim_hr_upper <- numeric(n_simulations_null)
  sim_proteins <- vector("list", n_simulations_null)

  for(sim in 1:n_simulations_null) {
    selected_proteins <- sample(truly_null_proteins, n_proteins, replace = FALSE)
    sim_proteins[[sim]] <- selected_proteins
  ...
  null_simulation_results[[as.character(n_proteins)]] <- data.frame(
    n_proteins = n_proteins,
    simulation = 1:n_simulations_null,
    c_index = sim_cindex,
    logrank_p = sim_logrank_p,
    hr = sim_hr,
    hr_lower = sim_hr_lower,
    hr_upper = sim_hr_upper,
    proteins = I(sim_proteins),
    stringsAsFactors = FALSE
  )

  valid_sims <- sum(!is.na(sim_cindex))
  valid_hr <- sum(!is.na(sim_hr))
  cat("Valid simulations:", valid_sims, "out of", n_simulations_null, "\n")
  cat("Valid HR calculations:", valid_hr, "out of", n_simulations_null, "\n")
  if(valid_sims > 0) {
    cat("Mean C-index:", round(mean(sim_cindex, na.rm = TRUE), 3),
        "(SD:", round(sd(sim_cindex, na.rm = TRUE), 3), ")\n")
    cat("Mean HR:", round(mean(sim_hr, na.rm = TRUE), 3),
    ...
    cat("Mean log-rank p-value:", round(mean(sim_logrank_p, na.rm = TRUE), 3),
        "(SD:", round(sd(sim_logrank_p, na.rm = TRUE), 3), ")\n")
    cat("Proportion with p < 0.05:", round(mean(sim_logrank_p < 0.05, na.rm = TRUE), 2), "\n")
  }
}

# Combine all results
all_null_results <- do.call(rbind, null_simulation_results)

# Create summary statistics
null_summary <- all_null_results %>%
  filter(!is.na(c_index)) %>%
  group_by(n_proteins) %>%
  summarise(
    mean_cindex = mean(c_index, na.rm = TRUE),
    ...
    sd_hr = sd(hr, na.rm = TRUE),
    mean_logrank_p = mean(logrank_p, na.rm = TRUE),
    sd_logrank_p = sd(logrank_p, na.rm = TRUE),
    prop_sig = mean(logrank_p < 0.05, na.rm = TRUE),
    .groups = "drop"
  )

cat("\n=== Truly Null Protein Simulation Summary ===\n")
print(null_summary)

# Save results
write.csv(all_null_results, file.path(wd$outCurr, "truly_null_protein_simulation_results.csv"), row.names = FALSE)
write.csv(null_summary, file.path(wd$outCurr, "truly_null_protein_simulation_summary.csv"), row.names = FALSE)
```
...
cpps_plot_data <- comparison_results %>%
  mutate(
    ...
    C_Index_SE = C_Index_SE  # Already has SE from concordance calculation
  ) %>%
...
random_plot_data <- sim_summary %>%
  ...
  mutate(
    ...
    C_Index = mean_cindex,
    C_Index_SE = sd_cindex / sqrt(n_valid),
    HR = mean_hr,
    HR_Lower = q25_hr,
    HR_Upper = q75_hr,
...
p_combined_cindex <- ggplot(combined_plot_data, aes(x = factor(N_Proteins), y = C_Index, fill = Model)) +
  ...
  scale_fill_manual(values = c("CPPS (Top Prognostic)" = "steelblue",
                                "Random (Non-prognostic)" = "lightcoral")) +
  theme_bw() +
  labs(title = "C-Index by Protein Count",
       subtitle = "Higher is better; dashed line = random chance (0.5)",
       x = "Number of Proteins",
       y = "C-Index",
       fill = "Model Type") +
...
p_combined_hr <- ggplot(combined_plot_data, aes(x = factor(N_Proteins), y = HR, color = Model, group = Model)) +
  ...
  scale_color_manual(values = c("CPPS (Top Prognostic)" = "steelblue",
                                 "Random (Non-prognostic)" = "lightcoral")) +
  theme_bw() +
  labs(title = "Hazard Ratio by Protein Count",
...
for(n_proteins in protein_counts) {
  ...
  # Get representative simulation info from representative_sims
  rep_sim_info <- representative_sims %>%
    filter(n_proteins == !!n_proteins)

  if(nrow(rep_sim_info) > 0) {
    # Extract information from representative_sims
    mean_sim_id <- rep_sim_info$simulation
    mean_logrank_p_val <- rep_sim_info$mean_logrank_p
    rep_logrank_p <- rep_sim_info$logrank_p
    rep_hr <- rep_sim_info$hr

    cat("Mean logrank_p for", n_proteins, "proteins:", round(mean_logrank_p_val, 3), "\n")
    cat("Selected simulation", mean_sim_id, "with logrank_p =", round(rep_logrank_p, 3), "\n")
    cat("  HR =", round(rep_hr, 3), "\n")

    # Get the exact proteins used in the representative simulation
    selected_proteins <- rep_sim_info$proteins_list[[1]]
    cat("  Using", length(selected_proteins), "proteins from representative simulation\n")
    ...
    risk_median <- median(random_km_df$risk_score, na.rm = TRUE)
    random_km_df <- random_km_df %>%
      mutate(
        risk_group = ifelse(risk_score >= risk_median, "High", "Low"),
    ...
    # Calculate log-rank p-value for verification (should match stored value)
    survdiff_check <- survdiff(Surv(time_to_death, death) ~ risk_group, data = random_km_df)
    recalc_logrank_p <- 1 - pchisq(survdiff_check$chisq, length(survdiff_check$n) - 1)

    cat("  Stored logrank_p from simulation:", round(rep_logrank_p, 4), "\n")
    cat("  Recalculated logrank_p:", round(recalc_logrank_p, 4), "\n")
    if(abs(rep_logrank_p - recalc_logrank_p) > 0.01) {
      cat("  WARNING: P-values differ by more than 0.01!\n")
    }
    ...
    random_km_plot <- ggsurvplot(
      ...
      xlab = "Time to Death (months)",
      ylab = "Overall Survival Probability",
      palette = c("grey", "lightcoral"),  # Different color to distinguish from CPPS
      legend.labs = c("Low Risk", "High Risk"),
      break.time.by = 12,
      ggtheme = theme_custom,
      surv.median.line = "hv"
...
pdf(file.path(wd$outCurr, "Random_mean_logrank_KM_plots.pdf"), width = 4, height = 20)
...
gridExtra::grid.arrange(
  ...
  top = grid::textGrob("Kaplan-Meier Plots for Representative Random Non-prognostic Protein Sets\n(Based on Mean Log-rank P-value)",
...
cat("- Random_mean_logrank_KM_plots.pdf\n")
cat("4 plots arranged vertically: 5, 10, 15, 20 proteins\n")
cat("Representative simulations selected based on mean log-rank p-value\n")
cat("===========================================\n")
```
