...
meta_c <- meta_c %>% 
  mutate(OS = as.numeric(difftime(event_date, date, units = "days")),
         ...
         death = case_when(death_status == "n" ~ 0,
                           death_status == "y" ~ 1)) %>% 
  select(HCI_cID, OS, death, psa, ldh, alk_phos, hemoGlo, alb)

# Save this 
meta_c_tmp <- meta_c %>% 
  dplyr::mutate(HCI_cID = gsub("-", "_", HCI_cID),
                HCI_cID = paste0("c_", HCI_cID))
write.csv(meta_c_tmp, file.path(wd$outData, "mCRPC_metadata.csv"))
```
...
c_stage_pro <- overlap_df_limma %>% 
  ...
  pull(OlinkID)
length(c_stage_pro)
write.csv(c_stage_pro, file.path(wd$outData, "mCRPC_proteins.csv"))
```
...
Get some stats of this pre-treatment cohor

```{r}
meta_pre_info <- meta_c %>% 
  filter(HCI_cID %in% meta_pre$HCI_cID)
table(meta_pre_info$death)
summary(meta_pre_info$OS)
```
...
clinical_vars <- c("psa", "ldh", "alk_phos", "hemoGlo", "alb")
clinical_results <- data.frame()

# Create survival data from meta_c with log10-transformed clinical variables
meta_c_surv <- meta_c %>%
  filter(HCI_cID %in% meta_pre2$HCI_cID) %>% 
  #filter(psa <= 100) %>% 
  mutate(psa = case_when(psa >100 ~ NA,
                         TRUE  ~ psa)) %>% 
...
clinical_vars <- c("psa", "log10_psa", "alk_phos", "log10_alp",
                   "ldh", "log10_ldh", "hemoGlo", "log10_hemo",
                   "alb", "log10_alb")
clinical_vars <- c("psa", "alk_phos",
                   "ldh", "hemoGlo",
                   "alb")
...
# Complete case of PSA, LDH and ALP
complete_ids <- meta_c_surv %>%
  filter(complete.cases(psa, ldh, alk_phos)) %>%
  pull(HCI_cID)

meta_in2 <- meta_in %>% filter(HCI_cID %in% complete_ids)
results <- analyze_proteins_multi(uni_sig, npx_in=tmp_npx, meta_in=meta_in2)
results <- left_join(results, df_id_pro)
...
Lets compute every single protein

```{r}
full_prot <- rownames(exp_mat_sub)
results_fullPro <- analyze_proteins_multi(full_prot, npx_in=tmp_npx, meta_in=meta_in2)
results_fullPro <- left_join(results_fullPro, df_id_pro)

results_fullPro <- results_fullPro %>% 
  mutate(
    Lower_Bound = round(Lower_Bound, 2),
    Upper_Bound = round(Upper_Bound, 2),
    bin_Lower_Bound = round(bin_Lower_Bound, 2),
    ...
    Sig_fdr_cont = case_when(
      ...
      TRUE ~ "Non"))
#wd$outCurr <- file.path(wd$output, "06_mCRPC_analysis")
write.csv(results_fullPro, file.path(wd$outCurr, "HR_values_mCRPC_specific_multi_pre-allProteins.csv"))
```
...
multi_renamed <- res_pre_multi %>%
  ...
  mutate(fdr_cont = formatC(fdr_cont, format = "e", digits = 2),
         P_Value = formatC(fdr_cont, format = "e", digits = 2),
         HR = round(HR, 2)) %>% 
  rename(
  ...
  mutate(
    Multi_FDR_num = as.numeric(Multi_FDR),
    Multi_Sig = case_when(
      Multi_FDR_num <= cox_fdr_val  ~ "Yes",
      TRUE ~ "No"))
...
How many are drug associated

```{r}
res_all2 %>%
  count(is_Dx, Multi_Sig) %>%
  mutate(percent = n / sum(n) * 100)

# Plot the pie chart
# Create pie data from your table
pie_data <- res_all2 %>%
  count(is_Dx, Multi_Sig) %>%
  mutate(
    is_Dx = case_when(is_Dx == "No" ~ "Dx.No",
                      is_Dx == "Yes" ~ "Dx.Yes"),
    Multi_Sig = case_when(Multi_Sig == "No" ~ "Prg.No",
                      Multi_Sig == "Yes" ~ "Prg.Yes"),
    Category = paste(is_Dx, Multi_Sig, sep = " / "),
    Category = factor(Category, levels=c("Dx.No / Prg.No", "Dx.Yes / Prg.No",
                                         "Dx.No / Prg.Yes", "Dx.Yes / Prg.Yes")),
    Percentage = round(n / sum(n) * 100, 1)
  ) %>%
  mutate(Label = paste0(Category, "\n", n, " (", Percentage, "%)"))

# Pie chart
p_pie <- ggplot(pie_data, aes(x = "", y = n, fill = Category)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y", start = 0) +
  geom_text(aes(label = Label),
            position = position_stack(vjust = 0.5),
            size = 4, fontface = "bold") +
  labs(title = "Distribution of is_Dx and Multi_Sig") +
  theme_void() +
  theme(legend.position = "none",
        plot.title = element_text(size = 14, face = "bold", hjust = 0.5)) +
  scale_fill_manual(values=c("#CBCBCB", "black", "#E7AB9C", "#C73E1D"))

p_pie

ggsave(file.path(wd$outCurr, "Drug_Pie.pdf"),
...
```{r}
df_plot <- res_all2 %>%
  select(Assay, OlinkID, Multi_HR, Multi_FDR_num, is_Dx, is_CS) %>% 
  mutate(log2HR = log2(Multi_HR),
         negLogFDR = -log10(Multi_FDR_num),
         Sig = Multi_FDR_num < cut_fdr,
         Category = case_when(
           Sig & is_Dx == "Yes" ~ "Significant + Drug Target",
...
drug_targets <- df_plot %>%
  ...
  select(Assay, Multi_HR, Multi_FDR_num, DrugBank.ID) %>%
  arrange(desc(Multi_HR)) 


drug_targets <- df_plot %>%
  filter(!is.na(DrugBank.ID) & Multi_FDR_num < cut_fdr) %>%
  select(Assay, Multi_HR, Multi_FDR_num, DrugBank.ID) %>%
  arrange(desc(Multi_FDR_num)) 
...
genes <- unique(drug_targets$Assay)
...
# The labels are too squished. 
set.seed(42)
pp <-
  ggraph(tg, layout = "fr") +
  ggraph::geom_edge_link(alpha = 0.3) +
  ggraph::geom_node_point(aes(color = is_query, size = pmax(deg, 1))) +
  #ggraph::geom_node_point(aes(color = is_query, size = 3)) +
  ggraph::geom_node_text(aes(label = name), repel = TRUE, check_overlap=TRUE, size = 6) +
  scale_color_manual(values = c("grey70", "tomato")) +
  scale_size(range = c(10, 20), name = "Degree") + 
  #guides(size = "none") +
  theme_void()
ggsave(file.path(wd$outCurr, "Drug_targets_BioGRRID.pdf"), 
     plot = pp, width = 14, height = 12)


# Fixing the text
set.seed(42)

pp <- ggraph(tg, layout = "kk") +
  # edges
  geom_edge_link(alpha = 0.3) +

  # POINTS: map size to degree (numeric); use a reasonable display range
  geom_node_point(aes(color = is_query, size = pmax(deg, 1))) +

  # small labels for non-query nodes - literal size (NOT aes)
  geom_node_text(
    data = function(x) filter(x, !is_query),
    aes(label = name),
    size =6,            # literal text size for non-query
    #label.padding = 1000,
    color = "black",
    repel = TRUE,
    check_overlap = FALSE
  ...
  # larger labels for query nodes - literal size
  geom_node_text(
    data = function(x) filter(x, is_query),
    aes(label = name),
    size = 10,            # literal text size for query
    color = "tomato",
    #fontface = "bold",
    repel = TRUE,
    check_overlap = FALSE
  ...
  # control point size mapping only
  scale_size_continuous(range = c(5, 20), name = "Degree") + 

  scale_color_manual(values = c("grey70", "tomato")) +
  theme_void()

ggsave(file.path(wd$outCurr, "Drug_targets_BioGRRID_zoom.pdf"),
       plot = pp, width = 14, height = 12)
...
drug_summary <- drug_targets %>%
  mutate(
    ...
    FDR_category = case_when(
      Multi_FDR_num <= 0.05 ~ "Very Significant (FDR ≤ 0.05)",
      Multi_FDR_num <= 0.1 ~ "Significant (0.05 < FDR ≤ 0.1)", 
      Multi_FDR_num <= 0.2 ~ "Moderately Significant (0.1 < FDR ≤ 0.2)",
...
drug_summary$Priority_Score <- log2(drug_summary$Multi_HR) * (-log10(drug_summary$Multi_FDR_num))
drug_summary <- drug_summary %>% arrange(desc(Priority_Score))

cat("=== Drug Target Prioritization ===\n")
print(drug_summary %>% select(Assay, Multi_HR, Multi_FDR_num, Priority_Score, HR_category))
...
topx_proteins <- res_all %>%
  ...
  arrange(desc(Multi_HR)) %>%
  #slice_head(n = 10) %>%
  dplyr::filter(Assay %in% c("SDC1", "GDF15", "FETUB")) %>% 
  select(OlinkID, Assay, Multi_HR, Multi_FDR)

cat("=== Top 3 Proteins by Hazard Ratio ===\n")
print(topx_proteins)

# # Temporarty to plot CD138 (SDC1) and GDF15
# topx_proteins <- res_all %>%
#   filter(Multi_Sig == "Yes") %>%
#   #arrange(Multi_FDR, desc(Multi_HR)) %>%
#   arrange(desc(Multi_HR)) %>%
#   filter(Assay %in% c("SDC1", "GDF15"))
#   select(OlinkID, Assay, Multi_HR, Multi_FDR)
```
...
for(i in 1:nrow(topx_proteins)) {
  ...
  result <- create_protein_km_plot(
    ...
    fdr_value = as.numeric(protein_info$Multi_FDR)
  )
...
custom_proteins_of_interest <- df_plot %>%
  filter(Sig == TRUE & !is.na(DrugBank.ID)) %>%
  select(Assay, OlinkID, Multi_HR, Multi_FDR_num) %>%
  arrange(desc(Multi_HR)) 
...
if(nrow(available_proteins) > 0) {
  ...
  # Print which proteins will be analyzed
  cat("=== Proteins selected for KM plots ===\n")
  cat("Total proteins:", nrow(available_proteins), "\n")
  print(available_proteins)

  # Check for duplicate protein names
  duplicate_names <- available_proteins %>%
    group_by(Assay) %>%
    filter(n() > 1) %>%
    ungroup()

  if(nrow(duplicate_names) > 0) {
    cat("\n=== Note: Multiple OlinkIDs found for some proteins ===\n")
    cat("These will be distinguished by their OlinkID in plots\n")
    print(duplicate_names %>% arrange(Assay))
  }
  ...
  # Generate KM plots for each custom protein (indexed by OlinkID)
  custom_km_results <- list()
  custom_km_plots <- list()

  # Iterate by row to get both OlinkID and Assay
  for(i in 1:nrow(available_proteins)) {

    olink_id <- available_proteins$OlinkID[i]
    protein_name <- available_proteins$Assay[i]

    cat("\n=== Processing Custom Protein:", protein_name, "(", olink_id, ") ===\n")

    # Get protein data by OlinkID
    protein_npx <- npx_custom %>%
      filter(OlinkID == olink_id) %>%
      select(sample_id, NPX)

    # Get HR and FDR if available (match by OlinkID)
    protein_results_match <- custom_protein_results %>%
      filter(OlinkID == olink_id)

    hr_value <- ifelse(nrow(protein_results_match) > 0,
                      protein_results_match$Multi_HR[1],
                      NA)
    fdr_value <- ifelse(nrow(protein_results_match) > 0,
                       as.numeric(protein_results_match$Multi_FDR)[1],
                       NA)

    # Create plot title with both Assay name and OlinkID
    plot_title <- paste0(protein_name, " (", olink_id, ")")

    # Create KM plot using the same function as before
    custom_result <- create_protein_km_plot(
      protein_data = protein_npx,
      protein_name = plot_title,  # Use combined name with OlinkID
      hr_value = ifelse(is.na(hr_value), 1.0, hr_value),
      fdr_value = ifelse(is.na(fdr_value), 1.0, fdr_value)
    )

    if(!is.null(custom_result)) {
      # Store results indexed by OlinkID
      custom_km_results[[olink_id]] <- custom_result
      custom_km_results[[olink_id]]$protein_name <- protein_name  # Keep original protein name
      custom_km_results[[olink_id]]$olink_id <- olink_id  # Store OlinkID

      custom_km_plots[[olink_id]] <- custom_result$plot
  ...
  # Summary of custom analysis (now indexed by OlinkID)
  if(length(custom_km_results) > 0) {
    custom_km_summary <- data.frame(
      OlinkID = names(custom_km_results),
      Protein = sapply(custom_km_results, function(x) x$protein_name),
      HR = sapply(names(custom_km_results), function(olink_id) {
        match_row <- custom_protein_results %>% filter(OlinkID == olink_id)
        ifelse(nrow(match_row) > 0, match_row$Multi_HR[1], NA)
      }),
      FDR = sapply(names(custom_km_results), function(olink_id) {
        match_row <- custom_protein_results %>% filter(OlinkID == olink_id)
        ifelse(nrow(match_row) > 0, match_row$Multi_FDR[1], NA)
      }),
...
top_cpps <- res_all2 %>%
  ...
  arrange(desc(Multi_HR)) %>%
  #slice_head(n = 10) 
  filter(Assay %in% c("SDC1", "EDF1", "CKAP4", "LMNB2", "LETM1", 
           "COX5B", "HIP1R", "HMOX2", "DPY30", 
           "WFDC2", "RCC1")) 

# genes <- c("LMNB2", "COX5B", "SDC1", "CKAP4", "WFDC2", "HMOX2", "LETM1", "DFFA", "DPY30", "HIP1R", "RBP2", "SDHB")

# filter(Assay %in% genes)
...
# # Manually picked drug targets (top 5)
# t1 <- res_all %>%
#   filter(OlinkID %in% c("OID21380", "OID31100", "OID30400", "OID21297", "OID20650"))
# 
# # Then select the top HR aside from these
# t2 <- res_all  %>%
#   filter(!OlinkID %in% t1$OlinkID) %>%
...
# top_cpps <- res_all2 %>%
#   filter(OlinkID %in% c(t1$OlinkID, t2$OlinkID))
# write.csv(top_cpps, file.path(wd$outCurr, "CPPS_top10_5drugs_5top.csv"))



cat("=== Proteins for CPPS Construction ===\n")
print(top_cpps %>% select(Assay, Multi_HR, Multi_FDR))

# Extract protein names for CPPS construction
cpps_proteins <- top_cpps$Assay
cat("\nCPPS will be constructed using these", nrow(top_cpps), "proteins:", paste(cpps_proteins, collapse = ", "), "\n")
```
...
meta_pre_tmp <- meta_pre %>% select(HCI_cID, final_sample_id) %>% distinct() %>% 
  left_join(meta_c_surv) %>% 
  select(-time_to_death, -death_event) %>% 
  rename(SampleID = final_sample_id)

# Join with the df_cpps
df_cpps_2 <- df_cpps %>%
  left_join(meta_pre_tmp) %>%
  dplyr::select(-SampleID)
write.csv(df_cpps_2, file.path(wd$outCurr, "CPPS_score_top5_proteins.csv"))
...
cpps_fit <- survfit(Surv(time_to_death, death) ~ CPPS_risk, data = df_cpps)
summary(cpps_fit)$table
with(df_cpps, tapply(time_to_death, CPPS_risk, range, na.rm = TRUE))
```
Plot the CPPS
...
# Log-rank test for CPPS risk groups
  cpps_survdiff <- survdiff(Surv(time_to_death, death) ~ CPPS_risk, data = df_cpps_km)
  cpps_logrank_p <- 1 - pchisq(cpps_survdiff$chisq, length(cpps_survdiff$n) - 1)

  cat("\nLog-rank Test:\n")
  print(cpps_survdiff)
  cat("Log-rank p-value:", formatC(cpps_logrank_p, format = "e", digits = 3), "\n")

  # Fit Cox proportional hazards model
  cpps_cox_km <- coxph(Surv(time_to_death, death) ~ CPPS_risk, data = df_cpps_km)
  cpps_summary_km <- summary(cpps_cox_km)

  # Extract HR and CI
  hr_value <- cpps_summary_km$conf.int[1, "exp(coef)"]
  hr_lower <- cpps_summary_km$conf.int[1, "lower .95"]
  hr_upper <- cpps_summary_km$conf.int[1, "upper .95"]
  hr_p <- cpps_summary_km$coefficients[1, "Pr(>|z|)"]

  cat("\nCox Regression Results:\n")
  cat("HR:", round(hr_value, 2), "[", round(hr_lower, 2), "-", round(hr_upper, 2), "]\n")
  cat("P-value:", formatC(hr_p, format = "e", digits = 3), "\n")

  # Kaplan-Meier analysis
  cpps_survfit <- survfit(Surv(time_to_death, death) ~ CPPS_risk, data = df_cpps_km)
  ...
  # Create survival plot with HR in title
  plot_title <- paste0("CPPS from 11 Proteins")
  ...
  # Save the plot
  pdf(file.path(wd$outCurr, paste0("CPPS_KM_survival with 11 prognostic", ".pdf")),
      width = 10, height = 8)
  print(cpps_km_plot)
  dev.off()
```

Get the median survival and summary of risk score

```{r}
df_cpps_km %>%
    dplyr::group_by(CPPS_risk) %>%
    dplyr::summarise(
        n = dplyr::n(),
        min = min(time_to_death, na.rm = TRUE),
        q1 = quantile(time_to_death, 0.25, na.rm = TRUE),
        median = median(time_to_death, na.rm = TRUE),
        mean = mean(time_to_death, na.rm = TRUE),
        q3 = quantile(time_to_death, 0.75, na.rm = TRUE),
        max = max(time_to_death, na.rm = TRUE),
        sd = sd(time_to_death, na.rm = TRUE)
    )
```
...
df_cpps_complete <- df_cpps %>% 
  left_join(select(meta_in, log10_psa, log10_alp, log10_ldh, sample_id) %>% rename(SampleID = sample_id))
...
```{r}
# Generate KM plots for different protein counts (5, 10, 15, 20)
...
for(n_proteins in protein_counts) {
  ...
  cat("\n===========================================\n")
  cat("Generating KM Plot for Top", n_proteins, "Proteins\n")
  cat("===========================================\n")

  # Select top N proteins
  top_cpps_km <- res_all2 %>%
    filter(Multi_Sig == "Yes") %>%
    arrange(desc(Multi_HR)) %>%
    slice_head(n = n_proteins)

  cat("Selected proteins:\n")
  print(top_cpps_km %>% select(Assay, Multi_HR, Multi_FDR))

  # Calculate CPPS score for these proteins
    df_pat_sum_km <- df_npx_long %>%
      filter(OlinkID %in% top_cpps_km$OlinkID) %>%
      select(final_sample_id, OlinkID, NPX) %>%
    left_join(top_cpps_km) %>%
    group_by(final_sample_id) %>%
    mutate(RS = Multi_Coefficient * NPX,
           Sum_rs = sum(RS)) %>%
    select(final_sample_id, Sum_rs) %>%
      ...
      rename(sample_id = final_sample_id) %>%
      left_join(meta_in) %>%
      select(sample_id, Sum_rs, death, OS, log10_psa, log10_alp, log10_ldh) %>%
      ungroup() %>%
      rename(SampleID = sample_id,
             CPPS = Sum_rs)

  # Merge CPPS scores with survival data
  df_cpps_km <- df_pat_sum_km %>%
    rename(time_to_death = OS) %>%
    na.omit()

  # Determine optimal CPPS cutoff using median
  cpps_median_km <- median(df_cpps_km$CPPS, na.rm = TRUE)

  # Create risk groups
  df_cpps_km <- df_cpps_km %>%
    mutate(
      CPPS_risk = ifelse(CPPS >= cpps_median_km, "High", "Low"),
      CPPS_risk = factor(CPPS_risk, levels = c("Low", "High"))
    )

  cat("\nRisk Group Distribution:\n")
  print(table(df_cpps_km$CPPS_risk))
  ...
  # Create survival plot with HR in title
  plot_title <- paste0("CPPS from Top ", n_proteins, " Proteins\n",
                      "HR = ", round(hr_value, 2),
  ...
  # Save the plot
  pdf(file.path(wd$outCurr, paste0("CPPS_KM_survival_top", n_proteins, ".pdf")),
      width = 10, height = 8)
  print(cpps_km_plot)
  dev.off()

  # Store in list
  km_plots_list[[paste0("Top_", n_proteins)]] <- list(
    plot = cpps_km_plot,
    data = df_cpps_km,
    survfit = cpps_survfit,
    cox_model = cpps_cox_km,
    hr = hr_value,
    hr_lower = hr_lower,
    hr_upper = hr_upper,
    p_value = hr_p,
...
cat("\n===========================================\n")
cat("KM Plot Generation Complete\n")
cat("Generated KM plots for:", paste(protein_counts, "proteins", collapse = ", "), "\n")
cat("===========================================\n")
```

### Model Comparison: Determining Optimal Protein Count

We compare models with different protein counts (5, 10, 15, 20) using multiple metrics to determine which provides the best predictive performance.
...
# Extract metrics for each protein count
for(n_proteins in protein_counts) {
  results <- km_plots_list[[paste0("Top_", n_proteins)]]

  # Calculate C-index (concordance index) for discrimination using adjusted continuous CPPS
  cox_model_continuous <- coxph(
    Surv(time_to_death, death) ~ log10_psa + log10_alp + log10_ldh + CPPS,
    data = results$data
  )

  # Calculate C-index using concordance function
  cindex_obj <- concordance(cox_model_continuous)

  cindex_value <- cindex_obj$concordance
  cindex_se <- sqrt(cindex_obj$var)  # SE from variance

  # Get AIC and BIC (lower is better)
  model_aic <- AIC(results$cox_model)
  model_bic <- BIC(results$cox_model)

  # Add to comparison table
  comparison_results <- rbind(comparison_results, data.frame(
    N_Proteins = n_proteins,
    HR = results$hr,
    HR_Lower = results$hr_lower,
    HR_Upper = results$hr_upper,
    Cox_P = results$p_value,
    LogRank_P = results$logrank_p,
    C_Index = cindex_value,
...
comparison_results <- comparison_results %>%
  mutate(
    # Higher C-index is better (rank 1 = highest)
    Rank_CIndex = rank(-C_Index),
    # Lower AIC is better (rank 1 = lowest)
    Rank_AIC = rank(AIC),
    # Lower BIC is better (rank 1 = lowest)
    Rank_BIC = rank(BIC),
    # Lower p-value is better (rank 1 = lowest)
    Rank_Pval = rank(Cox_P),
    # Higher HR is better for risk stratification (rank 1 = highest)
    Rank_HR = rank(-HR),
    # Overall rank (average of all ranks)
    Overall_Rank = (Rank_CIndex + Rank_AIC + Rank_BIC + Rank_Pval + Rank_HR) / 5
  ) %>%
...
print(comparison_results %>%
  select(N_Proteins, HR, C_Index, AIC, BIC, Cox_P, Overall_Rank) %>%
  mutate(across(where(is.numeric), ~round(., 3))))

cat("\n--- Best Model by Each Criterion ---\n")
cat("Best C-Index:", comparison_results$N_Proteins[which.max(comparison_results$C_Index)], "proteins (C-index =",
    round(max(comparison_results$C_Index), 3), ")\n")
cat("Best AIC:", comparison_results$N_Proteins[which.min(comparison_results$AIC)], "proteins (AIC =",
    round(min(comparison_results$AIC), 1), ")\n")
cat("Best BIC:", comparison_results$N_Proteins[which.min(comparison_results$BIC)], "proteins (BIC =",
    round(min(comparison_results$BIC), 1), ")\n")
cat("Best Overall Rank:", comparison_results$N_Proteins[which.min(comparison_results$Overall_Rank)],
    "proteins (rank =", round(min(comparison_results$Overall_Rank), 2), ")\n")

# Save comparison results
write.csv(comparison_results,
          file = file.path(wd$outCurr, "CPPS_model_comparison.csv"),
          row.names = FALSE)
...
p_cindex <- ggplot(comparison_results, aes(x = factor(N_Proteins), y = C_Index)) +
  geom_bar(stat = "identity", fill = "steelblue", alpha = 0.7) +
  geom_errorbar(aes(ymin = C_Index - C_Index_SE, ymax = C_Index + C_Index_SE),
                width = 0.2) +
  geom_text(aes(label = round(C_Index, 3)), vjust = -0.5, size = 3) +
  theme_bw() +
  labs(title = "C-Index by Protein Count",
       subtitle = "Higher is better (error bars show SE)",
       x = "Number of Proteins",
       y = "C-Index") +
  ylim(0, max(comparison_results$C_Index + comparison_results$C_Index_SE) * 1.1)

# Plot 2: AIC comparison
# p_aic <- ggplot(comparison_results, aes(x = factor(N_Proteins), y = AIC)) +
#   geom_bar(stat = "identity", fill = "coral", alpha = 0.7) +
#   geom_text(aes(label = round(AIC, 1)), vjust = -0.5, size = 3) +
#   theme_bw() +
#   labs(title = "AIC by Protein Count",
#        subtitle = "Lower is better",
#        x = "Number of Proteins",
#        y = "AIC") +
#   ylim(0, max(comparison_results$AIC) * 1.1)

# Plot 3: HR with confidence intervals
p_hr <- ggplot(comparison_results, aes(x = factor(N_Proteins), y = HR)) +
  geom_point(size = 3, color = "darkgreen") +
  geom_errorbar(aes(ymin = HR_Lower, ymax = HR_Upper), width = 0.2, color = "darkgreen") +
  geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
  geom_text(aes(label = round(HR, 2)), vjust = -1, size = 3) +
  theme_bw() +
  labs(title = "Hazard Ratio by Protein Count",
       subtitle = "Higher HR = better risk stratification",
       x = "Number of Proteins",
       y = "Hazard Ratio (95% CI)") +
  ylim(0, max(comparison_results$HR_Upper) * 1.2)

# Plot 4: P-value significance
p_pval <- ggplot(comparison_results, aes(x = factor(N_Proteins), y = -log10(Cox_P))) +
  geom_bar(stat = "identity", fill = "purple", alpha = 0.7) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") +
  geom_text(aes(label = sprintf("%.2e", Cox_P)), vjust = -0.5, size = 2.5) +
  theme_bw() +
  labs(title = "-log10(P-value) by Protein Count",
       subtitle = "Higher is more significant",
       x = "Number of Proteins",
       y = "-log10(P-value)") +
  ylim(0, max(-log10(comparison_results$Cox_P)) * 1.1)

# Combine plots - 3 plots vertically
comparison_plot <- p_cindex / p_hr / p_pval +
  plot_annotation(
    title = "CPPS Model Comparison Across Different Protein Counts",
...
# Save combined plot
ggsave(
  filename = file.path(wd$outCurr, "CPPS_model_comparison_plots.pdf"),
...
cat("\n===========================================\n")
cat("Model comparison results saved to:\n")
cat("- CPPS_model_comparison.csv\n")
cat("- CPPS_model_comparison_plots.pdf\n")
cat("===========================================\n")
```
...
for(n_proteins in protein_counts) {
  ...
  sim_cindex_se <- numeric(n_simulations)
  sim_logrank_p <- numeric(n_simulations)
  sim_hr <- numeric(n_simulations)
  sim_hr_lower <- numeric(n_simulations)
  sim_hr_upper <- numeric(n_simulations)
  ...
  for(sim in 1:n_simulations) {
    ...
    # Add survival + clinical data and restrict to complete-case cohort
    sim_df <- sim_df %>%
      left_join(
        meta_in %>%
          select(sample_id, OS, death, log10_psa, log10_alp, log10_ldh) %>%
          rename(SampleID = sample_id),
                by = "SampleID") %>%
      rename(time_to_death = OS) %>%
      filter(
        SampleID %in% df_cpps_complete$SampleID,
        !is.na(time_to_death),
        !is.na(death),
        !is.na(log10_psa),
        !is.na(log10_alp),
    ...
    # Calculate C-index and HR
    if(nrow(sim_df) > 10 && sum(sim_df$death) > 2) {
      # C-index using clinically-adjusted continuous risk_score
      cox_model_continuous <- coxph(
        Surv(time_to_death, death) ~ log10_psa + log10_alp + log10_ldh + risk_score,
        data = sim_df
      )
      ...
      sim_cindex[sim] <- cindex_obj$concordance
      sim_cindex_se[sim] <- sqrt(cindex_obj$var)  # SE from variance
      ...
      # Cox model for HR with error handling
      tryCatch({
        cox_model <- coxph(Surv(time_to_death, death) ~ risk_group, data = sim_df)
        if(!is.null(summary(cox_model)$conf.int) && nrow(summary(cox_model)$conf.int) > 0) {
          sim_hr[sim] <- summary(cox_model)$conf.int[1, 1]
          sim_hr_lower[sim] <- summary(cox_model)$conf.int[1, 3]
          sim_hr_upper[sim] <- summary(cox_model)$conf.int[1, 4]
        } else {
          sim_hr[sim] <- NA
          sim_hr_lower[sim] <- NA
          sim_hr_upper[sim] <- NA
  ...
  simulation_results[[as.character(n_proteins)]] <- data.frame(
    ...
    c_index = sim_cindex,
    c_index_se = sim_cindex_se,
    logrank_p = sim_logrank_p,
    hr = sim_hr,
    hr_lower = sim_hr_lower,
    hr_upper = sim_hr_upper
  )
  
  # Print summary statistics
  valid_sims <- sum(!is.na(sim_cindex))
  valid_hr <- sum(!is.na(sim_hr))
  cat("Valid simulations:", valid_sims, "out of", n_simulations, "\n")
  cat("Valid HR calculations:", valid_hr, "out of", n_simulations, "\n")
  if(valid_sims > 0) {
    cat("Mean C-index:", round(mean(sim_cindex, na.rm = TRUE), 3),
        "(SD:", round(sd(sim_cindex, na.rm = TRUE), 3), ")\n")
    if(valid_hr > 0) {
      cat("Mean HR:", round(mean(sim_hr, na.rm = TRUE), 3),
          "(SD:", round(sd(sim_hr, na.rm = TRUE), 3), ")\n")
      cat("HR range:", round(min(sim_hr, na.rm = TRUE), 3), "-",
          round(max(sim_hr, na.rm = TRUE), 3), "\n")
    }
...
sim_summary <- all_sim_results %>%
  ...
  summarise(
    ...
    q25_hr = quantile(hr, 0.25, na.rm = TRUE),
    q75_hr = quantile(hr, 0.75, na.rm = TRUE),
  ...
  mutate(
    mean_cindex_se = sd_cindex / sqrt(n_valid)  # SE of the mean
  )
...
for(n_top in cpps_protein_counts) {
  ...
  if(nrow(cpps_df) > 10 && sum(cpps_df$death) > 2) {
    ...
    # Calculate Cox model for HR
    cpps_cox <- coxph(Surv(time_to_death, death) ~ cpps_risk, data = cpps_df)
    cpps_hr <- summary(cpps_cox)$conf.int[1, 1]
    cpps_hr_lower <- summary(cpps_cox)$conf.int[1, 3]
    cpps_hr_upper <- summary(cpps_cox)$conf.int[1, 4]
    cpps_cox_p <- summary(cpps_cox)$coefficients[1, 5]
    ...
    cpps_comparison_results <- rbind(cpps_comparison_results, data.frame(
      ...
      hr = cpps_hr,
      hr_lower = cpps_hr_lower,
      hr_upper = cpps_hr_upper,
      cox_p = cpps_cox_p,
      stringsAsFactors = FALSE
    ))
...
p_combined2 <- ggplot() +
  # Add boxplots for simulation data
  geom_boxplot(data = sim_data_plot, 
               aes(x = x_pos, y = hr, group = n_proteins), 
               alpha = 0.7, fill = "lightblue", width = 0.3) +
  # Add quasirandom points for simulation data
  geom_quasirandom(data = sim_data_plot, 
                   aes(x = x_pos, y = hr), 
                   alpha = 0.7, size = 1.5, color = "grey20", width = 0.15) +
  ...
  geom_col(data = cpps_data_plot, 
           aes(x = x_pos, y = hr), 
           fill = "darkblue", alpha = 0.8, width = 0.3) +
  # Add reference lines
  geom_hline(yintercept = 1, color = "black", linetype = "dotted") +
  # Annotations for reference lines
  annotate("text", x = 0.6, y = 1, 
           label = "hr = 1", hjust = 0, vjust = -0.5, size = 3) +
  # Add legend manually using rectangles and text
  annotate("rect", xmin = 1.0, xmax = 1.2, ymin = 5.0, ymax = 5.3, 
...
### Combined Model Comparison: CPPS vs Random Proteins

Compare CPPS (top prognostic proteins) with random non-prognostic proteins using the same metrics and shared axes.

```{r}
library(patchwork)

# Prepare CPPS data for combined plots - use the comparison_results from the CPPS model comparison
cpps_plot_data <- comparison_results %>%
  mutate(
    Model = "CPPS (Top Prognostic)"
  ) %>%
  select(N_Proteins, Model, C_Index, C_Index_SE, HR, HR_Lower, HR_Upper, Cox_P)

# Prepare random protein summary data for combined plots
random_plot_data <- sim_summary %>%
  rename(N_Proteins = n_proteins) %>%
  mutate(
    Model = "Random (Non-prognostic)",
    C_Index = mean_cindex,
    C_Index_SE = mean_cindex_se,
    HR = mean_hr,
    HR_Lower = q25_hr,
    HR_Upper = q75_hr,
    Cox_P = mean_logrank_p
  )

# Combine datasets
combined_plot_data <- bind_rows(
  cpps_plot_data %>% select(N_Proteins, Model, C_Index, C_Index_SE, HR, HR_Lower, HR_Upper, Cox_P),
  random_plot_data %>% select(N_Proteins, Model, C_Index, C_Index_SE, HR, HR_Lower, HR_Upper, Cox_P)
) %>%
  mutate(Model = factor(Model, levels = c("CPPS (Top Prognostic)", "Random (Non-prognostic)")))

# Calculate shared axis limits
cindex_max <- max(combined_plot_data$C_Index + combined_plot_data$C_Index_SE, na.rm = TRUE)
hr_max <- max(combined_plot_data$HR_Upper, na.rm = TRUE)
pval_max <- max(-log10(combined_plot_data$Cox_P), na.rm = TRUE)

# Plot 1: C-Index comparison with shared axis
p_combined_cindex <- ggplot(combined_plot_data, aes(x = factor(N_Proteins), y = C_Index, fill = Model)) +
  ...
  scale_fill_manual(values = c("CPPS (Top Prognostic)" = "lightcoral",
                                "Random (Non-prognostic)" = "steelblue")) +
  theme_bw() +
  labs(title = "C-Index by Protein Count",
       subtitle = "Higher is better; error bars show SE; dashed line = random chance (0.5)",
       x = "Number of Proteins",
       y = "C-Index",
       fill = "Model Type") +
  ylim(0, cindex_max * 1.15) +
  theme(legend.position = "bottom")

# Plot 2: Hazard Ratio comparison with shared axis
p_combined_hr <- ggplot(combined_plot_data, aes(x = factor(N_Proteins), y = HR, color = Model, group = Model)) +
  ...
  geom_text(aes(label = round(HR, 2)),
            position = position_dodge(width = 0.4), vjust = -1.5, size = 2.5) +
  scale_color_manual(values = c("CPPS (Top Prognostic)" = "lightcoral",
                                 "Random (Non-prognostic)" = "steelblue")) +
  theme_bw() +
  labs(title = "Hazard Ratio by Protein Count",
       subtitle = "Higher HR = better risk stratification; dashed line = HR of 1",
       x = "Number of Proteins",
       y = "Hazard Ratio (95% CI)",
       color = "Model Type") +
  ylim(0, hr_max * 1.2) +
...
# Plot 3: P-value comparison with shared axis
p_combined_pval <- ggplot(combined_plot_data, aes(x = factor(N_Proteins), y = -log10(Cox_P), fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), alpha = 0.7, width = 0.7) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red", size = 0.5) +
  geom_text(aes(label = sprintf("%.1e", Cox_P)),
            position = position_dodge(width = 0.8), vjust = -0.5, size = 2) +
  scale_fill_manual(values = c("CPPS (Top Prognostic)" = "lightcoral",
                                "Random (Non-prognostic)" = "steelblue")) +
  theme_bw() +
  labs(title = "-log10(P-value) by Protein Count",
       subtitle = "Higher is more significant; dashed line = p=0.05",
       x = "Number of Proteins",
       y = "-log10(P-value)",
       fill = "Model Type") +
  ylim(0, pval_max * 1.15) +
  theme(legend.position = "bottom")

# Combine plots - 3 plots vertically with shared axes
combined_comparison_plot <- p_combined_cindex / p_combined_hr / p_combined_pval +
  plot_annotation(
    title = "Model Comparison: CPPS (Top Prognostic) vs Random (Non-prognostic) Proteins",
    subtitle = "Shared axes enable direct comparison between model types",
    theme = theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
                  plot.subtitle = element_text(hjust = 0.5, size = 10))
  )

print(combined_comparison_plot)

# Save combined plot
ggsave(
  filename = file.path(wd$outCurr, "CPPS_vs_Random_model_comparison.pdf"),
  plot = combined_comparison_plot,
  width = 4,
  height = 11
)
...
cat("\n===========================================\n")
cat("CPPS VS RANDOM PROTEIN COMPARISON\n")
cat("===========================================\n\n")

print(combined_plot_data %>%
  select(Model, N_Proteins, C_Index, HR, Cox_P) %>%
  arrange(N_Proteins, Model) %>%
  mutate(across(where(is.numeric) & !N_Proteins, ~round(., 3))))

cat("\n===========================================\n")
cat("Combined comparison plot saved to:\n")
cat("- CPPS_vs_Random_model_comparison.pdf\n")
cat("===========================================\n")
```

Generate Kaplan-Meier plots comparing CPPS (top prognostic) vs "median" performing random non-prognostic proteins:

```{r}
library(survminer)
library(gridExtra)

# For each protein count (5, 10, 15, 20), select the "median" performing random protein set
# based on the HR from the simulations, then create KM plots matching CPPS style

set.seed(789)  # For reproducibility

cpps_vs_random_km_plots <- list()

for(n_proteins in protein_counts) {
  cat("\n=== Creating KM plots for", n_proteins, "proteins ===\n")

  # Get all simulation results for this protein count
  sim_results_n <- all_sim_results %>%
    filter(n_proteins == !!n_proteins & !is.na(hr))

  # Find the simulation with median HR
  if(nrow(sim_results_n) > 0) {
    median_hr <- median(sim_results_n$hr, na.rm = TRUE)
    # Find the simulation closest to median HR
    median_sim_idx <- which.min(abs(sim_results_n$hr - median_hr))
    median_sim_id <- sim_results_n$simulation[median_sim_idx]

    cat("Median HR for", n_proteins, "proteins:", round(median_hr, 3), "\n")
    cat("Selected simulation", median_sim_id, "with HR =", round(sim_results_n$hr[median_sim_idx], 3), "\n")

    # Recreate the "median" random protein selection
    set.seed(123 + median_sim_id)  # Use same seed structure as original simulation
    selected_proteins <- sample(non_prognostic_proteins, n_proteins, replace = FALSE)
    ...
    # Add survival data
    random_km_df <- random_km_df %>%
      left_join(meta_in %>% select(sample_id, OS, death) %>% rename(SampleID = sample_id),
                by = "SampleID") %>%
      rename(time_to_death = OS) %>%
      filter(!is.na(time_to_death) & !is.na(death))

    # Calculate HR and survival stats for random proteins
    random_cox <- coxph(Surv(time_to_death, death) ~ risk_group, data = random_km_df)
    random_hr <- summary(random_cox)$conf.int[1, 1]
    random_hr_lower <- summary(random_cox)$conf.int[1, 3]
    random_hr_upper <- summary(random_cox)$conf.int[1, 4]

    # Fit survival curves
    random_survfit <- survfit(Surv(time_to_death, death) ~ risk_group, data = random_km_df)

    # Create plot title
    random_plot_title <- paste0("Random Non-prognostic (", n_proteins, " proteins)\n",
                                "HR = ", round(random_hr, 2),
                                " [", round(random_hr_lower, 2), "-", round(random_hr_upper, 2), "]")

    # Create KM plot for random proteins matching CPPS style
    random_km_plot <- ggsurvplot(
      fit = random_survfit,
      data = random_km_df,
      pval = TRUE,
      conf.int = TRUE,
      ...
      xlab = "Time to Death (months)",
      ylab = "Overall Survival Probability",
      palette = c("grey", "steelblue"),  # Different color to distinguish from CPPS
      legend.labs = c("Low Risk", "High Risk"),
      break.time.by = 12,
      ggtheme = theme_custom,
      surv.median.line = "hv"
    ...
    # Store the random KM plot
    cpps_vs_random_km_plots[[paste0("Random_", n_proteins)]] <- random_km_plot

  } else {
    cat("No valid simulations found for", n_proteins, "proteins\n")
  }
}

# Create PDF with all 4 random protein KM plots arranged horizontally

cat("\n===========================================\n")
cat("Creating Random Protein KM plots PDF\n")
cat("===========================================\n")

# Extract just the plot components (without risk tables for horizontal arrangement)
random_plot_list <- list()
for(n_proteins in protein_counts) {
  random_plot_obj <- cpps_vs_random_km_plots[[paste0("Random_", n_proteins)]]
  if(!is.null(random_plot_obj)) {
    random_plot_list[[paste0(n_proteins, "_proteins")]] <- random_plot_obj
  }
}

# Create single PDF with 4 plots arranged vertically (4 rows x 1 column)
pdf(file.path(wd$outCurr, "Random_median_KM_plots.pdf"), width = 4, height = 20)

# Arrange all 4 KM plots vertically
# Each plot will have its survival curve and risk table stacked
gridExtra::grid.arrange(
  random_plot_list[["5_proteins"]]$plot,
  random_plot_list[["5_proteins"]]$table,
  random_plot_list[["10_proteins"]]$plot,
  random_plot_list[["10_proteins"]]$table,
  random_plot_list[["15_proteins"]]$plot,
  random_plot_list[["15_proteins"]]$table,
  random_plot_list[["20_proteins"]]$plot,
  random_plot_list[["20_proteins"]]$table,
  ncol = 1,
  nrow = 8,
  heights = c(0.7, 0.3, 0.7, 0.3, 0.7, 0.3, 0.7, 0.3),
  top = grid::textGrob("Kaplan-Meier Plots for Median-Performing Random Non-prognostic Protein Sets",
...
cat("\n===========================================\n")
cat("Random protein KM plots saved to:\n")
cat("- Random_median_KM_plots.pdf\n")
cat("4 plots arranged vertically: 5, 10, 15, 20 proteins\n")
cat("===========================================\n")
```
...
df_cpps <- df_cpps %>% left_join(meta_in %>% select(sample_id, log10_psa, log10_alp, log10_ldh) %>% rename(SampleID = sample_id))
...
df_cpps_complete <- df_cpps %>%
  filter(!is.na(log10_psa) & !is.na(log10_alp) & !is.na(log10_ldh) & !is.na(CPPS) & !is.na(time_to_death) & !is.na(death))
...
library(rms)
cox_model_clinical <- cph(Surv(time_to_death, death) ~ log10_psa + log10_alp + log10_ldh, 
                         data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
cox_model_cpps_alone <- cph(Surv(time_to_death, death) ~ CPPS, 
                           data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
cox_model_cpps_combined <- cph(Surv(time_to_death, death) ~ log10_psa + log10_alp + log10_ldh + CPPS, 
                              data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
...
cox_model_clinical_nom <- cph(Surv(time_to_death, death) ~ log10_psa + log10_alp + log10_ldh, 
                             data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
cox_model_cpps_alone_nom <- cph(Surv(time_to_death, death) ~ CPPS, 
                               data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
cox_model_cpps_combined_nom <- cph(Surv(time_to_death, death) ~ log10_psa + log10_alp + log10_ldh + CPPS, 
                                  data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
...
pdf(file.path(wd$outCurr, "Nomogram_Combined_MultiTime.pdf"), height = 5, width = 10)
...
cat("Combined nomogram created: log10(PSA) + log10(ALP) + log10(LDH) CPPS with 24 and 36-month survival probabilities\n")
```
...
# Obj 5: Iterations of the risk socre

Here we want to asses the performance of the CPPS risk score in a few different iterations. perform the follwing

- Top 10 proteins by HR
- Top 10 proteins by FDR
- Top 10 proteins by HR & FDR 
- The 11 proteins of interest
- Random 10 non-prognostic proteins

For each we want to generate the cox-model and get the 24 and 36 month AUC. Also we want to generate the KM-plot from the resulting CPPS. 

```{r}
cat("=== Objective 5: CPPS iteration analysis ===\n")
...
prep_protein_table <- function(df_in) {
  df_in %>%
    select(OlinkID, Assay, Multi_HR, Multi_FDR_num, Multi_Coefficient) %>%
    filter(!is.na(OlinkID), !is.na(Multi_Coefficient)) %>%
    distinct(OlinkID, .keep_all = TRUE)
}

build_cpps_from_proteins <- function(protein_df) {
  # Try direct OlinkID matching first (most robust when IDs are consistent)
  n_match_olink <- sum(unique(df_npx_long$OlinkID) %in% unique(protein_df$OlinkID), na.rm = TRUE)
  cat("      OlinkID matches:", n_match_olink, "\n")

  if (n_match_olink > 0) {
    out_df <- df_npx_long %>%
      filter(OlinkID %in% protein_df$OlinkID) %>%
      select(final_sample_id, OlinkID, NPX) %>%
      left_join(protein_df %>% select(OlinkID, Multi_Coefficient), by = "OlinkID")
  } else {
    # Fallback to assay-name matching if OlinkIDs do not overlap
    npx_assay <- if ("Assay" %in% colnames(df_npx_long)) {
      df_npx_long %>% select(final_sample_id, Assay, NPX)
    } else {
      df_npx_long %>%
        left_join(df_id_pro %>% select(OlinkID, Assay) %>% distinct(), by = "OlinkID") %>%
        select(final_sample_id, Assay, NPX)
    }
    protein_weights <- protein_df %>%
      select(Assay, Multi_Coefficient) %>%
      distinct(Assay, .keep_all = TRUE)
    n_match_assay <- sum(unique(npx_assay$Assay) %in% unique(protein_weights$Assay), na.rm = TRUE)
    cat("      Assay matches:", n_match_assay, "\n")

    out_df <- npx_assay %>%
      filter(Assay %in% protein_weights$Assay) %>%
      left_join(protein_weights, by = "Assay")
  }

  cpps_df <- out_df %>%
    group_by(final_sample_id) %>%
    summarise(CPPS = sum(Multi_Coefficient * NPX, na.rm = TRUE), .groups = "drop")

  cat("      DEBUG final_sample_id (CPPS) head:",
      paste(head(unique(cpps_df$final_sample_id), 10), collapse = ", "), "\n")
  if ("sample_id" %in% colnames(meta_in)) {
    cat("      DEBUG meta_in$sample_id head:",
        paste(head(unique(meta_in$sample_id), 10), collapse = ", "), "\n")
  }
  if ("HCI_cID" %in% colnames(meta_pre2)) {
    cat("      DEBUG meta_pre2$HCI_cID head:",
        paste(head(unique(meta_pre2$HCI_cID), 10), collapse = ", "), "\n")
  }
  if ("final_sample_id" %in% colnames(meta_pre2)) {
    cat("      DEBUG meta_pre2$final_sample_id head:",
        paste(head(unique(meta_pre2$final_sample_id), 10), collapse = ", "), "\n")
  }

  # Attach metadata robustly across ID formats:
  # - direct final_sample_id -> meta_in$sample_id
  # - fallback final_sample_id -> HCI_cID via meta_pre2, then HCI_cID -> meta_in
  n_direct <- sum(cpps_df$final_sample_id %in% meta_in$sample_id, na.rm = TRUE)
  cat("      metadata direct sample_id matches:", n_direct, "\n")

  if (n_direct > 0) {
    cpps_meta <- cpps_df %>%
      rename(sample_id = final_sample_id) %>%
      left_join(meta_in, by = "sample_id")
  } else if (exists("lookup")) {
    # Preferred bridge when available: lookup maps HCI IDs <-> final_sample_id labels
    normalize_sample <- function(x) {
      x <- tolower(as.character(x))
      x <- gsub("-", "_", x)
      x <- gsub("^c_?", "", x)  # remove leading c or c_
      m <- stringr::str_match(x, "^(\\d{4})_?(\\d+)$")
      out <- ifelse(
        !is.na(m[, 1]),
        paste0("c_", m[, 2], "_", stringr::str_pad(m[, 3], width = 4, side = "left", pad = "0")),
        NA_character_
      )
      out
    }
    ...
    lookup_df <- tryCatch({
      if (is.atomic(lookup) && !is.null(names(lookup))) {
        tibble(
          final_sample_id = as.character(unname(lookup)),
          lookup_id = as.character(names(lookup))
        )
      } else if (is.data.frame(lookup)) {
        # Minimal fallback for table-like lookup objects
        nm <- colnames(lookup)
        tibble(
          final_sample_id = as.character(lookup[[nm[2]]]),
          lookup_id = as.character(lookup[[nm[1]]])
        )
      } else {
        tibble(final_sample_id = character(), lookup_id = character())
      }
    }, error = function(e) tibble(final_sample_id = character(), lookup_id = character()))

    lookup_df <- lookup_df %>%
      mutate(sample_id_norm = normalize_sample(lookup_id)) %>%
      distinct(final_sample_id, sample_id_norm)

    meta_norm <- meta_in %>%
      mutate(sample_id_norm = normalize_sample(sample_id))

    n_lookup_overlap <- sum(cpps_df$final_sample_id %in% lookup_df$final_sample_id, na.rm = TRUE)
    cat("      DEBUG lookup overlap (final_sample_id):", n_lookup_overlap, "\n")
    cat("      DEBUG lookup final_sample_id head:",
        paste(head(unique(lookup_df$final_sample_id), 10), collapse = ", "), "\n")
    cat("      DEBUG lookup sample_id_norm head:",
        paste(head(unique(lookup_df$sample_id_norm), 10), collapse = ", "), "\n")
    intersect_ids <- intersect(unique(lookup_df$sample_id_norm), unique(meta_norm$sample_id_norm))
    cat("      DEBUG lookup/meta normalized intersections:", length(intersect_ids), "\n")
    cat("      DEBUG lookup/meta intersection head:",
        paste(head(intersect_ids, 10), collapse = ", "), "\n")

    cpps_meta <- cpps_df %>%
      left_join(lookup_df, by = "final_sample_id") %>%
      left_join(meta_norm %>% select(sample_id, sample_id_norm, OS, death, log10_psa, log10_alp, log10_ldh),
                by = "sample_id_norm")

    n_lookup_meta <- sum(!is.na(cpps_meta$OS), na.rm = TRUE)
    cat("      metadata lookup bridge matches:", n_lookup_meta, "\n")
  } else if (all(c("final_sample_id", "HCI_cID") %in% colnames(meta_pre2)) &&
             any(c("HCI_cID", "sample_id") %in% colnames(meta_in))) {
    # Normalize HCI IDs to handle c2021_XXXX vs c_2021_XXXX naming differences
    normalize_hci <- function(x) {
      x <- tolower(as.character(x))
      x <- gsub("-", "_", x)
      x <- gsub("^c_?", "", x)  # remove leading c or c_
      m <- stringr::str_match(x, "^(\\d{4})_?(\\d+)$")
    ...
    meta_bridge <- meta_pre2 %>%
      select(final_sample_id, HCI_cID) %>%
      distinct() %>%
      mutate(HCI_norm = normalize_hci(HCI_cID))

    meta_surv <- if ("HCI_cID" %in% colnames(meta_in)) {
      meta_in %>%
        select(HCI_cID, OS, death, log10_psa, log10_alp, log10_ldh) %>%
        distinct() %>%
        mutate(HCI_norm = normalize_hci(HCI_cID))
    } else {
      meta_in %>%
        select(sample_id, OS, death, log10_psa, log10_alp, log10_ldh) %>%
        distinct() %>%
        mutate(HCI_norm = normalize_hci(sample_id))
    }

    cpps_key <- cpps_df %>%
      left_join(meta_bridge %>% select(final_sample_id, HCI_norm), by = "final_sample_id")
    n_norm_overlap <- sum(cpps_key$HCI_norm %in% meta_surv$HCI_norm, na.rm = TRUE)
    cat("      DEBUG normalized key overlap:", n_norm_overlap, "\n")
    cat("      DEBUG cpps HCI_norm head:",
        paste(head(unique(cpps_key$HCI_norm), 10), collapse = ", "), "\n")
    cat("      DEBUG meta_surv HCI_norm head:",
        paste(head(unique(meta_surv$HCI_norm), 10), collapse = ", "), "\n")

    cpps_meta <- cpps_df %>%
      left_join(meta_bridge, by = "final_sample_id") %>%
      left_join(meta_surv %>% select(-any_of(c("HCI_cID", "sample_id"))), by = "HCI_norm") %>%
      rename(sample_id = final_sample_id)
    n_hci <- sum(!is.na(cpps_meta$OS), na.rm = TRUE)
    cat("      metadata HCI_cID bridge matches:", n_hci, "\n")
  } else {
    cpps_meta <- cpps_df %>%
      rename(sample_id = final_sample_id)
    cat("      metadata bridge unavailable: no join performed\n")
  }

  # Ensure expected clinical/survival columns exist even if metadata join failed
  if (!"OS" %in% colnames(cpps_meta)) cpps_meta$OS <- NA_real_
  if (!"death" %in% colnames(cpps_meta)) cpps_meta$death <- NA_real_
  if (!"log10_psa" %in% colnames(cpps_meta)) cpps_meta$log10_psa <- NA_real_
  if (!"log10_alp" %in% colnames(cpps_meta)) cpps_meta$log10_alp <- NA_real_
  if (!"log10_ldh" %in% colnames(cpps_meta)) cpps_meta$log10_ldh <- NA_real_
...
run_cpps_iteration <- function(protein_df, iteration_name, set_label, file_stub, model_formula, eval_times = c(24, 36)) {
  df_iter <- build_cpps_from_proteins(protein_df)
  cat("  ", set_label, "-", iteration_name, ": proteins =", nrow(protein_df),
      "| rows after CPPS build =", nrow(df_iter), "\n")

  # If no data are available for this set, return NA summary and a placeholder KM plot
  if (nrow(df_iter) == 0) {
    dummy_df <- data.frame(
      time_to_death = c(1, 1),
      death = c(0, 0),
      CPPS_risk = factor(c("Low", "High"), levels = c("Low", "High"))
    )
    km_fit <- survfit(Surv(time_to_death, death) ~ CPPS_risk, data = dummy_df)
    km_plot <- ggsurvplot(
      fit = km_fit,
      data = dummy_df,
      pval = FALSE,
      ...
      risk.table = TRUE,
      risk.table.height = 0.3,
      title = paste0(set_label, ": ", iteration_name, " (no valid observations)"),
      xlab = "Time to Death (months)",
      ylab = "Overall Survival Probability",
      palette = c("grey", "#e41a1c"),
      legend.labs = c("Low Risk", "High Risk"),
    ...
    km_plot$plot <- km_plot$plot +
      labs(subtitle = "No overlapping non-missing NPX + survival rows for this set.")

    summary_df <- data.frame(
      Iteration = iteration_name,
      N_Proteins = nrow(protein_df),
      Proteins = paste(protein_df$Assay, collapse = ", "),
      N_Patients = 0,
      N_Patients_Cox_AUC = 0,
      Events = 0,
      Events_Cox_AUC = 0,
      HR_Continuous_CPPS = NA_real_,
      HR_Continuous_Lower95 = NA_real_,
      HR_Continuous_Upper95 = NA_real_,
      P_Continuous_CPPS = NA_real_,
      HR_High_vs_Low = NA_real_,
      HR_High_vs_Low_Lower95 = NA_real_,
      HR_High_vs_Low_Upper95 = NA_real_,
      P_High_vs_Low = NA_real_,
      AUC_24m = NA_real_,
      AUC_36m = NA_real_,
      stringsAsFactors = FALSE
    )

    return(list(summary = summary_df, km_plot = km_plot))
  }
  
  # Use complete clinical cases for Cox/AUC (same principle as df_cpps_complete)
  df_iter_cox <- df_iter %>%
    filter(
      !is.na(log10_psa),
      !is.na(log10_alp),
  ...
  # Optionally enforce overlap with df_cpps_complete IDs when available
  id_col <- if ("SampleID" %in% colnames(df_cpps_complete)) "SampleID" else if ("sample_id" %in% colnames(df_cpps_complete)) "sample_id" else NA_character_
  if (!is.na(id_col)) {
    cox_ids <- intersect(df_iter_cox$SampleID, as.character(df_cpps_complete[[id_col]]))
    if (length(cox_ids) > 0) {
      df_iter_cox <- df_iter_cox %>% filter(SampleID %in% cox_ids)
    }
  }
  cat("     rows for Cox/AUC =", nrow(df_iter_cox),
      "| events =", sum(df_iter_cox$death, na.rm = TRUE), "\n")

  cpps_cut <- median(df_iter$CPPS, na.rm = TRUE)
  df_iter <- df_iter %>%
    mutate(
      CPPS_risk = ifelse(CPPS >= cpps_cut, "High", "Low"),
      CPPS_risk = factor(CPPS_risk, levels = c("Low", "High"))
    )
  cpps_cut_cox <- median(df_iter_cox$CPPS, na.rm = TRUE)
  df_iter_cox <- df_iter_cox %>%
    mutate(
      CPPS_risk = ifelse(CPPS >= cpps_cut_cox, "High", "Low"),
      CPPS_risk = factor(CPPS_risk, levels = c("Low", "High"))
    )

  has_cox_data <- nrow(df_iter_cox) > 0 && sum(df_iter_cox$death, na.rm = TRUE) > 0
  if (has_cox_data) {
    cox_cont <- coxph(model_formula, data = df_iter_cox)
    cox_bin <- coxph(Surv(time_to_death, death) ~ CPPS_risk, data = df_iter_cox)
    lp_cont <- predict(cox_cont, type = "lp")
  } else {
    cox_cont <- NULL
    cox_bin <- NULL
    lp_cont <- NULL
  ...
  if (has_cox_data) {
    roc_iter <- timeROC(
      T = df_iter_cox$time_to_death,
      delta = df_iter_cox$death,
      marker = lp_cont,
      cause = 1,
      times = eval_times,
      iid = TRUE
    )
    # Match AUC extraction logic used in the main AUC section
    if (length(roc_iter$AUC) == length(eval_times) + 1) {
      auc_iter_results <- data.frame(
        Time_Months = eval_times,
        AUC_CPPS_Iter = round(roc_iter$AUC[2:(length(eval_times) + 1)], 3)
      )
    } else {
      auc_iter_results <- data.frame(
        Time_Months = eval_times,
        AUC_CPPS_Iter = round(roc_iter$AUC[1:length(eval_times)], 3)
      )
    }
    auc_vals <- auc_iter_results$AUC_CPPS_Iter
  } else {
    auc_vals <- rep(NA_real_, length(eval_times))
  }

  if (has_cox_data) {
    cont_summary <- summary(cox_cont)
    bin_summary <- summary(cox_bin)
    hr_value <- bin_summary$conf.int[1, "exp(coef)"]
    hr_lower <- bin_summary$conf.int[1, "lower .95"]
    hr_upper <- bin_summary$conf.int[1, "upper .95"]
  } else {
    cont_summary <- NULL
    bin_summary <- NULL
    hr_value <- NA_real_
  ...
  km_fit <- survfit(Surv(time_to_death, death) ~ CPPS_risk, data = df_iter)
  km_table <- summary(km_fit)$table
  median_low <- NA_real_
  median_high <- NA_real_
  if (!is.null(dim(km_table))) {
    if ("CPPS_risk=Low" %in% rownames(km_table)) {
      median_low <- as.numeric(km_table["CPPS_risk=Low", "median"])
    }
    if ("CPPS_risk=High" %in% rownames(km_table)) {
      median_high <- as.numeric(km_table["CPPS_risk=High", "median"])
    }
  }
  fmt_median <- function(x) ifelse(is.na(x), "NR", sprintf("%.2f", x))
  km_title <- paste0(
    set_label, ": ", iteration_name, " (", nrow(protein_df), " proteins)\n",
    "HR (High vs Low) = ", sprintf("%.2f", hr_value),
    " [", sprintf("%.2f", hr_lower), "-", sprintf("%.2f", hr_upper), "]"
  )
  km_subtitle <- paste0(
    "Median OS (months): Low=", fmt_median(median_low),
    " | High=", fmt_median(median_high),
    "\nProteins: ", paste(protein_df$Assay, collapse = ", ")
  )
  km_plot <- ggsurvplot(
    fit = km_fit,
  ...
  median_label <- paste0(
    "Median OS (months)\n",
    "Low: ", fmt_median(median_low), "\n",
    "High: ", fmt_median(median_high)
  )
  km_plot$plot <- km_plot$plot +
    labs(subtitle = km_subtitle) +
    annotate("text", x = Inf, y = Inf, label = median_label, hjust = 1.02, vjust = 1.1, size = 3.3) +
  ...
  summary_df <- data.frame(
    ...
    N_Proteins = nrow(protein_df),
    Proteins = paste(protein_df$Assay, collapse = ", "),
    N_Patients = nrow(df_iter),
    N_Patients_Cox_AUC = nrow(df_iter_cox),
    Events = sum(df_iter$death),
    Events_Cox_AUC = sum(df_iter_cox$death),
    HR_Continuous_CPPS = if (!is.null(cont_summary)) cont_summary$conf.int[1, "exp(coef)"] else NA_real_,
    HR_Continuous_Lower95 = if (!is.null(cont_summary)) cont_summary$conf.int[1, "lower .95"] else NA_real_,
    HR_Continuous_Upper95 = if (!is.null(cont_summary)) cont_summary$conf.int[1, "upper .95"] else NA_real_,
    P_Continuous_CPPS = if (!is.null(cont_summary)) cont_summary$coefficients[1, "Pr(>|z|)"] else NA_real_,
    HR_High_vs_Low = if (!is.null(bin_summary)) bin_summary$conf.int[1, "exp(coef)"] else NA_real_,
    HR_High_vs_Low_Lower95 = if (!is.null(bin_summary)) bin_summary$conf.int[1, "lower .95"] else NA_real_,
    HR_High_vs_Low_Upper95 = if (!is.null(bin_summary)) bin_summary$conf.int[1, "upper .95"] else NA_real_,
    P_High_vs_Low = if (!is.null(bin_summary)) bin_summary$coefficients[1, "Pr(>|z|)"] else NA_real_,
    AUC_24m = auc_vals[1],
    AUC_36m = auc_vals[2],
    stringsAsFactors = FALSE
  )
...
# Define iteration protein sets
sig_tbl <- res_all2 %>%
  filter(Multi_Sig == "Yes") %>%
  prep_protein_table()

top10_hr <- sig_tbl %>%
  arrange(desc(Multi_HR), Multi_FDR_num) %>%
  slice_head(n = 10)

top10_fdr <- sig_tbl %>%
  arrange(Multi_FDR_num, desc(Multi_FDR_num)) %>%
  slice_head(n = 10)


proteins_11_interest <- if (exists("top_cpps") && nrow(top_cpps) >= 11) {
  top_cpps %>% prep_protein_table()
} else {
  res_all2 %>%
    filter(Assay %in% c("SDC1", "EDF1", "CKAP4", "LMNB2", "LETM1",
                        "COX5B", "HIP1R", "HMOX2", "DPY30",
...
# Build random non-prognostic pool from limma table (proteins not included in res_all2)
non_CD <- res_all2 %>% pull(OlinkID)
non_prog_candidates <- limma_df %>%
  filter(!OlinkID %in% non_CD) %>%
  distinct(OlinkID, .keep_all = TRUE)

if (!"Assay" %in% colnames(non_prog_candidates)) {
  non_prog_candidates <- non_prog_candidates %>%
    mutate(Assay = OlinkID)
}

non_prog_candidates <- non_prog_candidates %>%
  mutate(
    Multi_HR = NA_real_,
    Multi_FDR_num = NA_real_,
    Multi_Coefficient = 1
  ) %>%
  prep_protein_table() %>%
  filter(!OlinkID %in% proteins_11_interest$OlinkID)

set.seed(2026)
random_n <- min(10, nrow(non_prog_candidates))
random10_non_prog <- non_prog_candidates %>%
  slice_sample(n = random_n)

iteration_sets <- list(
  list(name = "11 proteins of interest", set_label = "Set 1", file_stub = "iter_11_proteins_interest", proteins = proteins_11_interest),
  list(name = "Top 10 proteins by HR", set_label = "Set 2", file_stub = "iter_top10_hr", proteins = top10_hr),
  list(name = "Top 10 proteins by FDR", set_label = "Set 3", file_stub = "iter_top10_fdr", proteins = top10_fdr),
  list(name = "Random 10 non-prognostic proteins", set_label = "Set 4", file_stub = "iter_random10_nonprog", proteins = random10_non_prog)
)

model_modes <- list(
  list(mode = "CPPS_only", formula = Surv(time_to_death, death) ~ CPPS),
  list(mode = "Clinical_plus_CPPS", formula = Surv(time_to_death, death) ~ log10_psa + log10_alp + log10_ldh + CPPS)
)

all_iteration_results <- list()

for (mm in model_modes) {
  cat("\n=== Running model mode:", mm$mode, "===\n")

  mode_results_raw <- lapply(iteration_sets, function(x) {
    cat("\n--- Running:", x$name, "(", mm$mode, ") ---\n")
    run_cpps_iteration(
      protein_df = x$proteins,
      iteration_name = x$name,
      set_label = x$set_label,
      file_stub = x$file_stub,
      model_formula = mm$formula,
      eval_times = time_points
    )
  })

  mode_results <- bind_rows(lapply(mode_results_raw, function(x) x$summary)) %>%
    mutate(Model_Mode = mm$mode)
  mode_km_plots <- lapply(mode_results_raw, function(x) x$km_plot)

  # Save all 4 KM plots for this model mode on one 2x2 page
  km_pdf <- file.path(wd$outCurr, paste0("CPPS_KM_all_iterations_", mm$mode, ".pdf"))
  km_combined <- arrange_ggsurvplots(mode_km_plots, print = FALSE, ncol = 2, nrow = 2)
  pdf(km_pdf, width = 16, height = 12)
  print(km_combined)
  dev.off()

  write.csv(
    mode_results,
    file.path(wd$outCurr, paste0("CPPS_iteration_performance_summary_", mm$mode, ".csv")),
    row.names = FALSE
  )

  cat("\n=== CPPS Iteration Summary:", mm$mode, "===\n")
  print(mode_results %>%
          mutate(
            AUC_24m = round(AUC_24m, 3),
            AUC_36m = round(AUC_36m, 3),
            HR_High_vs_Low = round(HR_High_vs_Low, 3),
            P_High_vs_Low = signif(P_High_vs_Low, 3)
          ) %>%
          select(Model_Mode, Iteration, N_Proteins, N_Patients, N_Patients_Cox_AUC, Events, HR_High_vs_Low, P_High_vs_Low, AUC_24m, AUC_36m))

  all_iteration_results[[mm$mode]] <- mode_results
}

# Combined results across both model modes
iteration_results <- bind_rows(all_iteration_results)

write.csv(
  iteration_results,
  file.path(wd$outCurr, "CPPS_iteration_performance_summary_all_models.csv"),
  row.names = FALSE
)
```

## Iteration Protein Membership Plot

```{r}
# Create long table for all 4 iteration sets
set1_tbl <- proteins_11_interest %>% select(Assay) %>% mutate(Set = "Set1", Rank = row_number())
set2_tbl <- top10_hr %>% select(Assay) %>% mutate(Set = "Set2", Rank = row_number())
set3_tbl <- top10_fdr %>% select(Assay) %>% mutate(Set = "Set3", Rank = row_number())
set4_tbl <- random10_non_prog %>% select(Assay) %>% mutate(Set = "Set4", Rank = row_number())

membership_long <- bind_rows(set1_tbl, set2_tbl, set3_tbl, set4_tbl) %>%
  distinct()

# Build complete matrix so absent proteins can be shown in grey
all_proteins <- membership_long %>% distinct(Assay)
all_sets <- tibble(Set = c("Set1", "Set2", "Set3", "Set4"))

membership_mat <- tidyr::crossing(all_proteins, all_sets) %>%
  left_join(membership_long %>% mutate(In_Set = TRUE), by = c("Assay", "Set")) %>%
  mutate(
    In_Set = ifelse(is.na(In_Set), FALSE, In_Set),
    Rank_Label = ifelse(In_Set, as.character(Rank), "")
  )

# Order proteins by set-specific ranks in sequence:
# Set1 first, then Set2..Set4 for tie-breaking while preserving previous order.
rank_wide <- membership_long %>%
  select(Assay, Set, Rank) %>%
  distinct() %>%
  tidyr::pivot_wider(names_from = Set, values_from = Rank, names_prefix = "Rank_")

protein_order <- membership_mat %>%
  group_by(Assay) %>%
  summarise(Set_Count = sum(In_Set), .groups = "drop") %>%
  left_join(rank_wide, by = "Assay") %>%
  mutate(
    Rank_Set1 = ifelse(is.na(Rank_Set1), Inf, Rank_Set1),
    Rank_Set2 = ifelse(is.na(Rank_Set2), Inf, Rank_Set2),
    Rank_Set3 = ifelse(is.na(Rank_Set3), Inf, Rank_Set3),
    Rank_Set4 = ifelse(is.na(Rank_Set4), Inf, Rank_Set4)
  ) %>%
  arrange(Rank_Set1, Rank_Set2, Rank_Set3, Rank_Set4, desc(Set_Count), Assay) %>%
  pull(Assay)

membership_mat <- membership_mat %>%
  mutate(
    Assay = factor(Assay, levels = rev(protein_order)),
    Set = factor(Set, levels = c("Set1", "Set2", "Set3", "Set4"))
  )

# Left annotation: 4 columns (one per set), colored when protein is present in that set
annot_colors <- c("Present" = "black", "Absent" = "white")

annot_mat <- membership_mat %>%
  mutate(Annot_Fill = ifelse(In_Set, "Present", "Absent"))

p_annot <- ggplot(annot_mat, aes(x = Set, y = Assay, fill = Annot_Fill)) +
  geom_tile(color = "white", linewidth = 0.3) +
  geom_text(aes(label = Rank_Label), size = 2.4, color = "white") +
  scale_fill_manual(values = annot_colors, drop = FALSE) +
  theme_bw() +
  theme(
...
# AUC bar plot for 24 and 36 months with HR [95% CI] labels
set_map <- tibble(
  Set = c("Set1", "Set2", "Set3", "Set4"),
  Iteration = c(
    "11 proteins of interest",
    "Top 10 proteins by HR",
    "Top 10 proteins by FDR",
    "Random 10 non-prognostic proteins"
  )
...
for (mode_name in names(all_iteration_results)) {
  iteration_results_mode <- all_iteration_results[[mode_name]]

  auc_plot_df <- iteration_results_mode %>%
    left_join(set_map, by = "Iteration") %>%
    mutate(Set = factor(Set, levels = c("Set1", "Set2", "Set3", "Set4"))) %>%
    select(Set, AUC_24m, AUC_36m, HR_High_vs_Low, HR_High_vs_Low_Lower95, HR_High_vs_Low_Upper95) %>%
    pivot_longer(cols = c(AUC_24m, AUC_36m), names_to = "Timepoint", values_to = "AUC") %>%
    mutate(
      Timepoint = factor(Timepoint, levels = c("AUC_24m", "AUC_36m"),
                         labels = c("24 months", "36 months"))
    )

  hr_label_df <- iteration_results_mode %>%
    left_join(set_map, by = "Iteration") %>%
    mutate(
      Set = factor(Set, levels = c("Set1", "Set2", "Set3", "Set4")),
      HR_Label = paste0(
        "HR ", sprintf("%.2f", HR_High_vs_Low),
        " [", sprintf("%.2f", HR_High_vs_Low_Lower95),
        ", ", sprintf("%.2f", HR_High_vs_Low_Upper95), "]"
      )
    ) %>%
    select(Set, HR_Label)

  y_text <- min(0.99, max(auc_plot_df$AUC, na.rm = TRUE) + 0.06)

  p_auc <- ggplot(auc_plot_df, aes(x = Set, y = AUC, fill = Timepoint)) +
    geom_col(position = position_dodge(width = 0.75), width = 0.65, color = "black", linewidth = 0.2) +
    geom_text(
      data = hr_label_df,
      aes(x = Set, y = y_text, label = HR_Label),
      inherit.aes = FALSE,
      size = 3
    ) +
    scale_fill_manual(values = c("24 months" = "#4C78A8", "36 months" = "#F58518")) +
...
# p_points workflow kept for optional reuse:
# p_points <- ggplot(membership_mat, aes(x = Set, y = Assay)) +
#   geom_point(aes(fill = In_Set),
#              shape = 21, size = 3.2, stroke = 0.25, color = "black") +
...
#   )
#
# p_membership_bottom <- p_annot + p_points + plot_layout(widths = c(0.22, 1))
# p_membership_combined <- p_auc / p_membership_bottom +
#   plot_layout(heights = c(0.7, 1))

  p_membership_combined <- p_auc / p_annot +
    plot_layout(heights = c(0.7, 1)) +
    plot_annotation(
      title = paste0("Protein Membership Across 4 Iteration Sets (", mode_name, ")"),
      subtitle = "Top: AUC at 24 and 36 months with HR [95% CI]. Bottom: set membership annotation (black = present)."
    )

  print(p_membership_combined)

  ggsave(
    file.path(wd$outCurr, paste0("CPPS_iteration_set_membership_plot_", mode_name, ".pdf")),
    p_membership_combined,
    width = 10,
    height = 9
  )
}
...
```{r}
save(
  df_cpps_complete, # CPPS score of selcted proteins
  res_all2, # Result of multivariate prognositc
  file = file.path(wd$outData, "05_data.Rdata"))
```
