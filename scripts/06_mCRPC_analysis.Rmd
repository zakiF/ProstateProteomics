...
run_cpps_iteration <- function(protein_df, iteration_name, file_stub, model_formula, eval_times = c(24, 36)) {
  df_iter <- build_cpps_from_proteins(protein_df)
  
  # Use the same complete-case cohort as main AUC/Cox analysis
  cox_ids <- intersect(df_iter$SampleID, df_cpps_complete$SampleID)
  df_iter_cox <- df_iter %>% filter(SampleID %in% cox_ids)
  ...
  km_title <- paste0(
    iteration_name, " (", nrow(protein_df), " proteins)\n",
  ...
  # KM plot is written to the currently open PDF device (opened per model mode)
  print(km_plot)
  ...
  data.frame(
    ...
    HR_Continuous_CPPS = cont_summary$conf.int[1, "exp(coef)"],
    HR_Continuous_Lower95 = cont_summary$conf.int[1, "lower .95"],
    HR_Continuous_Upper95 = cont_summary$conf.int[1, "upper .95"],
    P_Continuous_CPPS = cont_summary$coefficients[1, "Pr(>|z|)"],
    HR_High_vs_Low = bin_summary$conf.int[1, "exp(coef)"],
    HR_High_vs_Low_Lower95 = bin_summary$conf.int[1, "lower .95"],
    HR_High_vs_Low_Upper95 = bin_summary$conf.int[1, "upper .95"],
    P_High_vs_Low = bin_summary$coefficients[1, "Pr(>|z|)"],
    AUC_24m = auc_vals[1],
    AUC_36m = auc_vals[2],
    stringsAsFactors = FALSE
  )
...
top10_hr_fdr <- sig_tbl %>%
  mutate(
    rank_combined = -log10(Multi_FDR_num) * abs(log(pmax(Multi_HR, 1e-10)))
  ) %>%
  arrange(rank_combined) %>%
  slice_head(n = 10) %>%
  select(-rank_combined)
...
iteration_sets <- list(
  list(name = "Top 10 proteins by HR", file_stub = "iter_top10_hr", proteins = top10_hr),
  list(name = "Top 10 proteins by FDR", file_stub = "iter_top10_fdr", proteins = top10_fdr),
  list(name = "Top 10 proteins by HR + FDR", file_stub = "iter_top10_hr_fdr", proteins = top10_hr_fdr),
  list(name = "11 proteins of interest", file_stub = "iter_11_proteins_interest", proteins = proteins_11_interest),
  list(name = "Random 10 non-prognostic proteins", file_stub = "iter_random10_nonprog", proteins = random10_non_prog)
)
...
for (mm in model_modes) {
  ...
  pdf(km_pdf, width = 10, height = 8, onefile = TRUE)

  mode_results <- lapply(iteration_sets, function(x) {
  ...
  mode_results <- bind_rows(mode_results) %>%
...
```{r}
# Create long table for all 5 iteration sets
set1_tbl <- top10_hr %>% select(Assay) %>% mutate(Set = "Set1", Rank = row_number())
set2_tbl <- top10_fdr %>% select(Assay) %>% mutate(Set = "Set2", Rank = row_number())
set3_tbl <- top10_hr_fdr %>% select(Assay) %>% mutate(Set = "Set3", Rank = row_number())
set4_tbl <- proteins_11_interest %>% select(Assay) %>% mutate(Set = "Set4", Rank = row_number())
set5_tbl <- random10_non_prog %>% select(Assay) %>% mutate(Set = "Set5", Rank = row_number())

membership_long <- bind_rows(set1_tbl, set2_tbl, set3_tbl, set4_tbl, set5_tbl) %>%
  distinct()

# Build complete matrix so absent proteins can be shown in grey
all_proteins <- membership_long %>% distinct(Assay)
all_sets <- tibble(Set = c("Set1", "Set2", "Set3", "Set4", "Set5"))
...
# Set1 first, then Set2..Set5 for tie-breaking while preserving previous order.
...
protein_order <- membership_mat %>%
  ...
  mutate(
    ...
    Rank_Set5 = ifelse(is.na(Rank_Set5), Inf, Rank_Set5)
  ) %>%
  arrange(Rank_Set1, Rank_Set2, Rank_Set3, Rank_Set4, Rank_Set5, desc(Set_Count), Assay) %>%
  pull(Assay)

membership_mat <- membership_mat %>%
  mutate(
    Assay = factor(Assay, levels = rev(protein_order)),
    Set = factor(Set, levels = c("Set1", "Set2", "Set3", "Set4", "Set5"))
  )

# Left annotation: 5 columns (one per set), colored when protein is present in that set
...
for (mode_name in names(all_iteration_results)) {
  ...
  auc_plot_df <- iteration_results_mode %>%
    ...
    mutate(Set = factor(Set, levels = c("Set1", "Set2", "Set3", "Set4", "Set5"))) %>%
...
#   plot_layout(heights = c(0.7, 1))
  ...
  p_membership_combined <- p_auc / p_annot +
    plot_layout(heights = c(0.7, 1)) +
    plot_annotation(
      title = paste0("Protein Membership Across 5 Iteration Sets (", mode_name, ")"),
